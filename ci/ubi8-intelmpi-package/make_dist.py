...
# Intel MPI root directory
mpi_root="/opt/intel/oneapi/mpi/2021.18"

# Determine Truchas version
this_dir = os.path.dirname(os.path.abspath(__file__))
root_dir = os.environ.get("TRUCHAS_SOURCE_DIR", os.path.realpath(this_dir + "/.."))
...
def copy_lib(lib, msg):
    filename = os.path.basename(lib)
    if filename in copied_libraries:
        return
    copied_libraries.add(filename)
    print(msg % filename)
    run("cp %s %s/lib" % (lib, dist))
...
# Copy the main binary and Intel MPI executables
run("cp %s %s/bin" % (tbin, dist))
run("cp genre %s/bin" % (dist))
run("cp vizre %s/bin" % (dist))
run("cp %s/bin/mpiexec.hydra %s/bin/mpiexec" % (mpi_root, dist))
run("cp %s/bin/hydra_bstrap_proxy %s/bin/" % (mpi_root, dist))
run("cp %s/bin/hydra_pmi_proxy %s/bin/" % (mpi_root, dist))
run("cp %s/opt/mpi/libfabric/lib/libfabric.so.* %s/lib/" % (mpi_root, dist))
run("cp -r %s/opt/mpi/libfabric/lib/prov %s/lib/" % (mpi_root, dist))

# Copy the truchas launch script
run("cp /tools/truchas-binary-intelmpi-Linux.sh %s/bin/truchas" % dist)
...
# Copy a README
run("cp %s/README-binary-Linux.md %s/README.md" % (this_dir, dist))
...
for b in [tbin, "genre", "vizre", "mpiexec", "hydra_bstrap_proxy", "hydra_pmi_proxy"]:
