...
image=${IMAGE_TAG:-truchas-ubi8-intelmpi-build:2026.0}
jobs=${JOBS:-$(nproc)}
...
case "${command}" in
  package)
    output_dir=${OUTPUT_DIR:-${PWD}/ubi8-intelmpi-package-output}
    ...
    ;;
  test)
    output_dir=${OUTPUT_DIR:-${PWD}/ubi8-intelmpi-ctest-output}
    mkdir -p "${output_dir}"
    podman run --rm --shm-size=1g --user 0 --env HOME=/home/swuser --env JOBS="${jobs}" \
      --volume "${script_dir}:/tools:ro,Z" \
      --volume "${source_snapshot}:/source:ro,Z" \
      --volume "${output_dir}:/output:Z" \
      "${image}" bash -o pipefail -c \
    ...
    ;;
  smoke)
    package_dir=${PACKAGE_DIR:-${PWD}/ubi8-intelmpi-package-output}
