...
-----------------------------------------------------------------------------
   Purpose:
      ...
      3) PostProcessor Visualisation Checker TestCase(s)
      4) PostProcessor Diagnostic Checker TestCase(s)
  
      by implementing RunLoader, PPLoader, PPVisualisationLoader, PPDiagnosticLoader
  
   Public Interface:
  
      T = TruchasPostProcessorLoader(dir,norerun,parallel_env,np,
                                     compiler,compile_mode,truchas_bin,
                                     truchas_exe)
      T.getSuite()
  
   Contains: TruchasPostProcessorLoader
...
from PPVisualisationLoader import PPVisualisationLoader
from PPDiagnosticLoader    import PPDiagnosticLoader
from Interrogators         import DirectoryInterrogator

class TruchasPostProcessorLoader(unittest.TestLoader):
    """
    ...
    def getSuite(self):
	    ...
	    #when run these will create multiple Truchas restart output dirs
            ...
            suite8, self.logdir = X.loadRestartRunTests()
            ...
            for exefile in os.listdir(truchas_bin):
         	if os.path.isfile(os.path.join(truchas_bin,exefile)) and exefile != 'truchas':
        		...
        		debug        = 1
                        ...
                        loader       = TruchasPostProcessorLoader(no_rerun,
                                                                  srcdir,
                                                                  currdir,
                                                                  debug,
                                                                  parallel_env,
