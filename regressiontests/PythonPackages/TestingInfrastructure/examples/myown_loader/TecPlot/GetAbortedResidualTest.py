...
class GetAbortedResidualTest(TruchasCapabilityTest):
    ...
    def testPlotNLRAborts(self):
        "tests plotting non-linear residual aborts with TecPlot"
	...
	mylogdir  = self.basicrunspecs.logdir
        ...
        for nlr in self.nlrs:
            ...
            pltfile  = 'tecplotnlr%d' %(idx)
	    tecplotfile  = os.path.join(mylogdir,pltfile)
            self.plotter.plotFields(vis      = 'TecPlot',
                                    plotfile = tecplotfile, 
                                    mesh     = self.testdata[0].meshes[0],
                                    fields   = nlr.vlist,
				    format   = 'ascii')
            
    def testPlotLRAborts(self):
        "tests plotting linear residual aborts with TecPlot"
