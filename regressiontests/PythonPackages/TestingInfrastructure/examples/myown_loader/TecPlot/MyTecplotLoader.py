...
-----------------------------------------------------------------------------
   ...
   Public Interface:
      ...
      T = MyTecplotLoader(dir,norerun,parallel_env,np,
                          compiler,compile_mode,truchas_bin,
                          truchas_exe)
      T.getSuite()
  
   Contains: MyTecplotLoader
...
class MyTecplotLoader(unittest.TestLoader):
    """
    ...
    def getSuite(self):
        ...
        if self.no_rerun:
            ...
            #now check the TecPlot results of GetAbortedResidual TestCase 

            Z      = PPVisualisationLoader(self.currdir,
                                           self.debug,
                                           RunSpecs)
            
            suite3 = Z.loadPPTecPlotChecker(pattern='GetAbortedResidualTest.py')
    ...
    for exefile in os.listdir(truchas_bin):
 	if os.path.isfile(os.path.join(truchas_bin,exefile)) and exefile != 'truchas':
		...
		debug        = 1
                ...
                loader       = MyTecplotLoader(no_rerun,
                                               srcdir,
                                               currdir,
                                               debug,
                                               parallel_env,
