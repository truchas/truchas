...
class CapabilityTest(TruchasCapabilityTest):
    ...
    def setTolerances(self):
        ...
        self.tol['Stress_abs']    = 2.0e0
        self.tol['Strain_rel']    = 1.0e-8
        self.tol['Strain_abs']    = 1.0e-10
        self.tol['Temp_rel']      = 1.0e-6

    def setDataStores(self):
        "defines testdata and goldendata output directories needed by this Capability TestCase"

        self.testdirs   = ['tm_pc_both_ds_output']
        self.goldendirs = ['tm_pc_both_ds_golden','tm_pc_both_ds_pgolden']

    def setDefinitions(self):
        "Define the data fields"

        self.whole_mesh   = self.testdata[0].getRegion(['Default'])
        self.sigma        = self.testdata[0].getField(field='sigma', cycle=216, region= self.whole_mesh)
        self.epsilon      = self.testdata[0].getField(field='epsilon', cycle=216, region= self.whole_mesh)
        self.epstherm     = self.testdata[0].getField(field='epstherm', cycle=216, region= self.whole_mesh)
        self.temp         = self.testdata[0].getField(field='T', cycle=216, region= self.whole_mesh)
        if 'serial' in self.basicrunspecs.parallel_env:
            self.sigma_ref    = self.goldendata[0].getField(field='sigma', cycle=216, region= self.whole_mesh)
            self.epsilon_ref  = self.goldendata[0].getField(field='epsilon', cycle=216, region= self.whole_mesh)
            self.epstherm_ref = self.goldendata[0].getField(field='epstherm', cycle=216, region= self.whole_mesh)
            self.temp_ref     = self.goldendata[0].getField(field='T', cycle=216, region= self.whole_mesh)
        else:
            self.sigma_ref    = self.goldendata[1].getField(field='sigma', cycle=216, region= self.whole_mesh)
            self.epsilon_ref  = self.goldendata[1].getField(field='epsilon', cycle=216, region= self.whole_mesh)
            self.epstherm_ref = self.goldendata[1].getField(field='epstherm', cycle=216, region= self.whole_mesh)
            self.temp_ref     = self.goldendata[1].getField(field='T', cycle=216, region= self.whole_mesh)
...
#  The new material method for solid mechanics phase change does not separate phase
#  change strain from the thermal strain, so this test is not useful
...
#   a nonlinear equation.
         ...
         self.logger.write("   T, Calc, reference, error")
         for j in range(ncells):
             T = self.temp.data[j]
             if T >= Ti:
                 epsref = (T-T0) * cte1
...
#                 epsref = epsref + a * (T - Th) + b * (T**2 - Th**2)
             ...
             self.logger.write("  %13.7e %13.7e %13.7e %13.7e\n" %(T, self.epstherm.data[j,0], epsref, diff))
             if diff > error: error = diff
    ...
    def testFinalStress(self):
         ...
         if errorzz > self.tol['Stress_abs']:
             fail = fail + 1
             self.logger.write("  Final sigzz not within %12e: FAIL\n" %(self.tol['Stress_abs']))
    ...
    def testFinalStrain(self):
         ...
         if errorzz > self.tol['Strain_rel']:
             fail = fail + 1
             self.logger.write("  Final epszz not within %12e: FAIL\n" %(self.tol['Strain_rel']))
