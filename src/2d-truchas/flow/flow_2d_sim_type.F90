...
!! FLOW_2D_SIM_TYPE
!!
!! This module defines FLOW_2D_SIM, which owns the mesh, material distribution,
!! Stokes flow model, solver, output schedule, and output writer for a
!! two-dimensional incompressible-flow simulation.
!!
...
module flow_2d_sim_type
  ...
  type, public :: flow_2d_sim
    ...
    type(stokes_2d_solver), pointer :: solver => null()
...
contains
  ...
  subroutine init(this, env, params, stat, errmsg)
    ...
    integer :: i, rlev
    real(r8), allocatable :: initial_velocity(:,:), temperature(:), vfrac(:,:)
    ...
    if (size(this%tout) == 0 .or. any(this%tout <= this%t_init) .or. &
        any(this%tout(2:) <= this%tout(:size(this%tout)-1))) then
      stat = 1
    ...
    call env%simlog%info('Initial time: ' // real_to_string(this%t_init))
    call env%simlog%info('Initial time step: ' // real_to_string(this%solver%initial_time_step()))
    if (size(this%tout) == 1) then
      write(message,'(a,es11.4)') 'Output schedule: 1 time at ', this%tout(1)
    else
      write(message,'(a,i0,a,es11.4,a,es11.4)') 'Output schedule: ', size(this%tout), ' times from ', &
          this%tout(1), ' through ', this%tout(size(this%tout))
    end if
  ...
  contains
    ...
    subroutine construct_flow_model(params, stat, errmsg)
      ...
      call this%model%init_core(env, this%mesh, bc_params, stat, errmsg, &
    ...
    subroutine read_material_regions(stat, errmsg)
      ...
      character(:), allocatable :: region_names(:), region_materials(:)
      integer :: i

      stat = 0
      if (params%is_sublist('material-regions')) then
        call get_material_region_names(params%sublist('material-regions'), matl_name, stat, errmsg, &
            region_names, region_materials)
        if (stat /= 0) return
        do i = 1, size(region_names)
          call env%simlog%info('Region "' // trim(region_names(i)) // '": material="' // &
              trim(region_materials(i)) // '".')
        end do
  ...
  function real_to_string(value) result(string)
    real(r8), intent(in) :: value
    character(:), allocatable :: string
    character(32) :: buffer

    write(buffer,'(es11.4)') value
    string = trim(buffer)
  end function
  ...
  subroutine write_solution(this, time)
    ...
    call this%solver%get_reduced_volume_fractions(this%matl_dist, vfrac)
    call this%output%write_solution(time, pressure, velocity, this%temporal_output, flow_active, vfrac)
  end subroutine
