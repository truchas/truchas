...
- [x] **External thermal rate.**  Add the general cell-integrated external
  enthalpy-rate interface to `t2d_thermal_model`/`t2d_thermal_solver`; zero remains the
...
- [x] **Coupled solver and sim.**  `t2d_flow_thermal_integrator` owns retry and
  sequencing, while `t2d_flow_thermal_sim`/main own setup, output scheduling,
