...
"""Four-process reference regression for pressure-driven fluid/VOID flow."""
...
def run_case(executable, input_file, mpiexec):
    output_dir = Path(tempfile.mkdtemp(prefix="flow_free_surface_pressure_4p_"))
...
def compare_field(actual, reference, name, step):
    if name in ("velocity", "pressure"):
        ...
        if np.any(~actual_active) and not np.isnan(actual[~actual_active]).all():
            raise RuntimeError(f"step {step}: {name} inactive cells are not NaN")
...
def main():
    ...
    expected_times = np.array([0.0, 3.0, 6.0, 10.0])
    ...
    print("PASS: pressure-driven fluid/VOID flow agrees with four-process reference")
