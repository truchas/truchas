...
"""Analytic hydrostatic regression for a water/VOID free surface."""
...
def run_case(executable, input_file, mpiexec):
    output_dir = pathlib.Path(tempfile.mkdtemp(prefix="flow_free_surface_hydrostatic_4p_"))
    command = [
        mpiexec,
        "-n",
        "4",
...
def check_solution(data):
    ...
    for step, expected_time in enumerate((0.0, 1.0)):
        ...
        expected_water = (centers[:, 1] < 0.5).astype(float)
...
def main():
    ...
    print("PASS: hydrostatic water/VOID free-surface state is preserved")
