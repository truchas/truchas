...
/* parameters for Telluride [lally]
 *
 * Many parameters have been changed for Telluride.  We want to quiet   *
 * the Chaco output, as we aren't interested.  Most importantly, Chaco  *
 * CAN NOT free the data structure passed in, as we create it in F90,   *
 * not in C (FREE_GRAPH must be FALSE).                                 */
...
char     *PARAMS_FILENAME = "/dev/null";	/* File of parameter changes */
...
int       DEBUG_PARAMS = 0;	/* Debug flag for reading parameter file (0..2) */
