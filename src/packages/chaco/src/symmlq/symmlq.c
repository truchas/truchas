...
L10:
    ...
    x[1] = eps;
    y[1] = 1.;
    chaco_daxpy_(&c__1, &c_b4, &x[1], &c__1, &y[1], &c__1);
    if (y[1] > 1.) {
	goto L10;
    }
    eps *= 2.;
...
/*     y and beta1 will be zero if b = 0. */
    chaco_dcopy_(n, &b[1], &c__1, &y[1], &c__1);
    chaco_dcopy_(n, &b[1], &c__1, &r1[1], &c__1);
    if (*precon) {
...
/*        b1  = zero */
/*     end if */
    beta1 = chaco_ddot_(n, &r1[1], &c__1, &y[1], &c__1);
/*     See if msolve is symmetric. */
    if (*checka && *precon) {
	msolve_(n, &y[1], &r2[1], a, &vwsqrt[1], &work[1]);
	s = chaco_ddot_(n, &y[1], &c__1, &y[1], &c__1);
	t = chaco_ddot_(n, &r1[1], &c__1, &r2[1], &c__1);
	z = (d__1 = s - t, abs(d__1));
	epsa = (s + eps) * pow_dd_chaco(&eps, &c_b18);
	if (z > epsa) {
	    *istop = 7;
	    goto L900;
	}
...
/*     See if aprod  is symmetric. */
	...
	s = chaco_ddot_(n, &y[1], &c__1, &y[1], &c__1);
	t = chaco_ddot_(n, &v[1], &c__1, &r2[1], &c__1);
	z = (d__1 = s - t, abs(d__1));
	epsa = (s + eps) * pow_dd_chaco(&eps, &c_b18);
	if (z > epsa) {
	    *istop = 6;
...
/*     y and beta will be zero or very small if b is an eigenvector. */
    d__1 = -(*shift);
    chaco_daxpy_(n, &d__1, &v[1], &c__1, &y[1], &c__1);
    alfa = chaco_ddot_(n, &v[1], &c__1, &y[1], &c__1);
    d__1 = -alfa / beta1;
    chaco_daxpy_(n, &d__1, &r1[1], &c__1, &y[1], &c__1);
/*     Make sure  r2  will be orthogonal to the first  v. */
    z = chaco_ddot_(n, &v[1], &c__1, &y[1], &c__1);
    s = chaco_ddot_(n, &v[1], &c__1, &v[1], &c__1);
    d__1 = -z / s;
    chaco_daxpy_(n, &d__1, &v[1], &c__1, &y[1], &c__1);
    chaco_dcopy_(n, &y[1], &c__1, &r2[1], &c__1);
    if (*precon) {
	msolve_(n, &r2[1], &y[1], a, &vwsqrt[1], &work[1]);
    }
    oldb = beta1;
    beta = chaco_ddot_(n, &r2[1], &c__1, &y[1], &c__1);
    if (beta < 0.) {
	*istop = 8;
	goto L900;
    }
...
/*     See if the local reorthogonalization achieved anything. */
    denom = sqrt(s) * chaco_dnrm2_(n, &r2[1], &c__1) + eps;
    s = z / denom;
    t = chaco_ddot_(n, &v[1], &c__1, &r2[1], &c__1) / denom;
...
/* L200: */
	...
	chaco_dcopy_(n, &v[1], &c__1, &w[1], &c__1);
    }
/*     ------------------------------------------------------------------ 
*/
/*     Main iteration loop. */
...
/* Computing 2nd power */
    ...
    d__1 = -alfa / beta;
    chaco_daxpy_(n, &d__1, &r2[1], &c__1, &y[1], &c__1);
    chaco_dcopy_(n, &r2[1], &c__1, &r1[1], &c__1);
    chaco_dcopy_(n, &y[1], &c__1, &r2[1], &c__1);
    if (*precon) {
	msolve_(n, &r2[1], &y[1], a, &vwsqrt[1], &work[1]);
    }
...
/* Computing 2nd power */
	...
	*rnorm = cgnorm;
	chaco_daxpy_(n, &zbar, &w[1], &c__1, &x[1], &c__1);
    } else {
	*rnorm = lqnorm;
    }
    if (*goodb) {
/*        Add the step along  b. */
	...
	chaco_daxpy_(n, &bstep, &y[1], &c__1, &x[1], &c__1);
    }
/*     ================================================================== 
*/
