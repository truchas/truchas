...
@(#)++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
@(#)
@(#)    $RCSfile: VF_CalcVF_Analytic.c,v $
@(#)    $Revision: 1.3 $  $Date: 2005/09/08 16:41:25 $  $Author: mwglass $
@(#)    $Source: /cvsroot/chaparral/chaparral/vflib/VF_CalcVF_Analytic.c,v $
@(#)
@(#)    DESCRIPTION: 
@(#) 
@(#)    From "On the Form Factor between Two Polygons", 
@(#)    P. Schroder and P. Hanrahan, Siggraph  1993. 
@(#) 
@(#)    See http://graphics.stanford.edu/papers/formfactor/ffpaper.ps.Z
@(#)    See http://graphics.stanford.edu/papers/formfactor/libff.tar.Z
@(#)
@(#)--------------------------------------------------------------------------
*/
...
#define M_PIF           ((double)M_PI)
#define HALF            ((double)0.5)
#define THREEHALF       ((double)1.5)
#define ONEQUARTER      ((double)0.25)
#define ONEEIGHTTH      ((double)0.125)
#define ONESIXTEENTH    ((double)0.0625)
#define ONETHIRTYSECOND ((double)0.03125)
 ...
 */
#define ilog(j,x,y)     \
  (ATAN2(y,x)+(((j)==1)&&((y)<0)&&((x)<=0)?(double)(2*M_PIF):\
                (((((j)==2)&&(((y)>0)||(((y)==0)&&((x)<0))))||\
...
IDilog( double z[2] )
{
  ...
  double omega = 2 * ATAN2( z[Y], 1 - z[X] );
  double theta = 2 * ATAN2( z[Y], z[X] );
  double result = 0;
...
double
G( double a, double b, double c, double t )
{
  double qt = ( a * t + b ) * t + c;
  double qpt = 2 * a * t + b;
...
double
H( double a, double b, double c, double t )
{
  double at = a * t;
  double qt = ( at + b ) * t + c;
  double twoa = 2 * a;
 ...
 */
void
Pair( double c[ALLCOEFF][2],
      double p1[3], double p2[3],
      double q1[3], double q2[3] )
{
  double dp[3], dq[3], qp[3];
  double l;

  memset(c, 0, ALLCOEFF*2*sizeof(double));
  diff( dp, p2, p1 );
  ...
  /*if( c[CONST][X] > mach[2] ){     changed by MWG */
  if( c[CONST][X] > 1.0e-7 ){
 ...
 */
int
Bilinear( double c[ALLCOEFF][2] )
{
  /*if( c[PLANE][X] * c[PLANE][X] < mach[2] ){     changed by MWG */
  if( c[PLANE][X] * c[PLANE][X] < 1.0e-7 ){
    /*
    ...
    if( c[CONST][X] < 1.0e-7 ){
      ...
      double ts = sin( acos( c[THETA][X] ) );
      double ps = sin( acos( c[PHI][X] ) );
...
double
IntegralPlanar( double c[ALLCOEFF][2] )
{
  double tmp[2], tmp2[2]; /* for evalint{p|m}() macro */
  double den[2], f[2];
 ...
 */
int
Lcis( double xax[2], double yax[2], double rad[2], double cnt[2] )
{
  /*
   * see whether a circle at c with radius r
   * intersects the x or y axis
   */
  double cabs = cmagsqr( cnt );
  double rabs = cmagsqr( rad );
  double xdisc = cnt[X] * cnt[X] - cabs + rabs;
  double ydisc = cnt[Y] * cnt[Y] - cabs + rabs;
  int ret = 0;
 ...
 */
int
LogSelect( double rad[2], double cnt[2], double psi[2] )
{
  double tmp; /* seghit() needs this one */
  double xax[2], yax[2];
  int sl;
...
RM( double z[2] )
{
  double t2_1[2]; /* conj(t^2 - 1) */
  double t2_1_2[2]; /* conj(t^2 - 1)^2 */
  double t_1[2], tp1[2]; /* t-1, t+1 */
  double quot;
...
double
IDilogPath( int k, double rad[2], double cnt[2], double c[ALLCOEFF][2] )
{
  double xax[2], yax[2], end[2], start[2];
  double tmp; /* seghit() needs this one */
  double parg, p[2];
  double f;
...
double
ILogPart( int p1, double z[2], double c[ALLCOEFF][2] )
{
  double rad[2], cnt[2]; /* radius, center */
  double tmp[2], tmp2[2]; /* the macros need these */
  double fac[2]; /* generic factor holding slot */
  double denom[2]; /* generic denominator holding slot */
  double f = 0; /* the final result */
  double quot; /* for quotients */
  double sum[2];
  int p2, p3;
  
  /* after adding this, it's giving wrong answers for triangles */
  /* ADDED BY MWG */
  /*if (z[X]==1 || z[X]==-1) return 0;*/
  /*
 ...
 */
double
ILogIntegral( double c[ALLCOEFF][2], double s )
{
  static double unit[2] = { 1, 0 };
  double mc17[2]; /* -c[17] */
  double mc18[2]; /* -c[18] */
  double logu, logl;
  /*
   ...
   * and end of the path
   */
  double tanu = -ATAN2( 2 * s + c[1][X] * c[2][X] + c[3][X],
                     sqrt( ( c[10][X] * c[2][X] + c[11][X] ) *
                          c[2][X] + c[12][X] ) );
  double tanl = -ATAN2( 2 * s + c[3][X], sqrt( c[12][X] ) );
   ...
   */
  ku = (int)floor((tanu-logu)/M_PIF+HALF);
  kl = (int)floor((tanl-logl)/M_PIF+HALF);
...
double
VF_CalcVF_Analytic(Poly *poly_i, Poly *poly_j)
{
  int    i, j, ii, jj, np_i, np_j;
  double p[2][3], q[2][3], vf = 0.0;
  double c[ALLCOEFF][2];
  double PolyArea(Poly *poly);

  np_i = poly_i->np;
  np_j = poly_j->np;
  for( i = 0, ii = 1; i < np_i; i++, ii = ( i + 1 ) % np_i ){
    p[0][0] = poly_i->p[ i].x; 
    p[0][1] = poly_i->p[ i].y; 
    p[0][2] = poly_i->p[ i].z;
    p[1][0] = poly_i->p[ii].x; 
    p[1][1] = poly_i->p[ii].y; 
    p[1][2] = poly_i->p[ii].z;
    for( j = 0, jj = 1; j < np_j; j++, jj = ( j + 1 ) % np_j ){
      q[0][0] = poly_j->p[ j].x; 
      q[0][1] = poly_j->p[ j].y; 
      q[0][2] = poly_j->p[ j].z;
      q[1][0] = poly_j->p[jj].x; 
      q[1][1] = poly_j->p[jj].y; 
      q[1][2] = poly_j->p[jj].z;
      Pair( c, p[0], p[1], q[0], q[1] );
      /*if( fabs( c[1][X] ) > mach[2] ){     changed by MWG */
      if( fabs( c[1][X] ) > 1.0e-10 ){
        vf -= c[1][X] * Integral( c );
      }
    }
  }
  vf /= (double)(M_PIF*8.0)*VF_PolyArea(poly_i);
  return (vf);
}
