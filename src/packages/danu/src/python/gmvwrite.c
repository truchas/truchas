...
void gmvwrite_nodev_data(void *nndes, void *x, void *y, void *z)
 {
   int tmpnndes, i, j;
   float *tempx, *tempy, *tempz, *tempxyz;
   double *tempx64, *tempy64, *tempz64, *tempxyz64;
   long tmpnndes64, longcount, totnodes;
...
{
  int i, tmpnumcells, tmpnumtop, *tmpdaughters;
  long ilong, tmpnumcells64, tmpnumtop64, *tmpdaughters64;
...
void gmvwrite_cell_type(char cell_type[], int nverts, void *nodes)
 {
   int tmpnverts, count;
   int *tempnodes, *nodesptr;
   long *tempnodes64, *nodesptr64;
...
void gmvwrite_general_cell_type(char cell_type[], int nverts[], int nfaces,
			        void *nodeids)
 {
   int sumverts=0, count, *tempnverts, tmpnfaces, *tempnodeids;
   long *tempnodeids64, *nodesptr64;
   int *nodesptr;
...
void gmvwrite_face_data(int nverts, void *nodeids, void *cellid1, 
                   void *cellid2)
{
   int  tempnverts, *tempnodeids, tmpcellid1, tmpcellid2;
   long *tempnodeids64, *nodesptr64, tmpcellid164, tmpcellid264;
   int *nodesptr, count;
...
{
   int  tempnverts, *tempnodeids, tempcellid, tempfacepe, tempoppface,
        tempoppfacepe;
   long *tempnodeids64, *nodesptr64, tempcellid64, tempoppface64;
   int *nodesptr, count;
...
void gmvwrite_velocity_data(int data_type, void *u, void *v, void *w)
 {
  ...
  double *tempu64, *tempv64, *tempw64;
...
void gmvwrite_group_data(char groupname[], int data_type, int numgrp, 
     void *group_data)
{
  int tmpdata_type, tmpnumgrp, *tmpdata, *groupptr, count;
  long *tmpdata64, *groupptr64;
...
void gmvwrite_subvars_name_data(int data_type, int numelem, char varname[], 
 ...
 {
  int tmpdata_type, tmpnumelem, *tmpdata, *subvarids;
  long count, npts;
  long *tmpdata64, *subvarids64;
  float *tmpvdata;
  double *tmpvdata64;
...
{
  int tmpdata_type, tmpnumghst, *tmpdata, *ghostptr, count;
  long *tmpdata64, *ghostptr64;
...
void gmvwrite_vector_name(char varname[], int data_type, int numcomps,  
                          int cnameflag)
 {
  int tmpdata_type, tmpnumcomps, tmpcnameflag;

  tmpdata_type = data_type;
  tmpnumcomps = numcomps;
  tmpcnameflag = cnameflag;

   if (filetype == IEEE_F)
     {
      fwrite(varname, sizeof(char), charsize_out, fp);
      fwrite(&tmpdata_type, INT32, 1, fp);
      fwrite(&tmpnumcomps, INT32, 1, fp);
      fwrite(&tmpcnameflag, INT32, 1, fp);
    }
   else
      fprintf(fp,"%s %d %d %d\n",varname,tmpdata_type,tmpnumcomps,tmpcnameflag);
}

/* --------------------------------------------------------- */

void gmvwrite_vector_compname(char subname[])
{
   if (filetype == IEEE_F)
      fwrite(subname, sizeof(char), charsize_out, fp);
   else
...
void gmvwrite_vector_data(int data_type, int ncomps, void *vids)
{
  int *tmpdata;
  long numthings, i;
  long count, npts;
