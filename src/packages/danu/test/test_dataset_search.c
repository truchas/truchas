...
int main(int argc, char ** argv)
{
    ...
    if ( nlinks > 0 ) {
        ...
        datasets = DANU_MALLOC(char *,nlinks);
        size = DANU_MALLOC(size_t,nlinks);

        for(i=0;i<nlinks;i++) {
            size[i] = str_size;
            datasets[i] = DANU_MALLOC(char,str_size);
        }

        status = danu_group_get_datasets(gid,nlinks,size,datasets,&num_found);
