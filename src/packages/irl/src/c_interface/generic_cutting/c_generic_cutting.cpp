...
#include "src/c_interface/generic_cutting/c_generic_cutting.h"
...
void c_getNormMoments_Tet_PlanarSep_Vol(
    const c_Tet* a_tet,
    const c_PlanarSep* a_planar_separator, double* a_moments_to_return){
	  assert(a_tet != nullptr);
	  assert(a_tet->obj_ptr != nullptr);
...
void c_getNormMoments_Pyrmd_PlanarSep_Vol(
    const c_Pyrmd* a_pyramid,
    const c_PlanarSep* a_planar_separator, double* a_moments_to_return){
...
void c_getNormMoments_CapDod_LLLL_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_LLLL* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_LLLT_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_LLLT* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_LTLT_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_LTLT* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_LLTT_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_LLTT* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_LTTT_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_LTTT* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_TTTT_LocSepLink_TagAccVM_SepVol(
    const c_CapDod_TTTT* a_capped_dodecahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapOcta_LLL_LocSepLink_TagAccVM_SepVol(
    const c_CapOcta_LLL* a_capped_octahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
...
void c_getNormMoments_CapOcta_LLT_LocSepLink_TagAccVM_SepVol(
    const c_CapOcta_LLT* a_capped_octahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapOcta_LTT_LocSepLink_TagAccVM_SepVol(
    const c_CapOcta_LTT* a_capped_octahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapOcta_TTT_LocSepLink_TagAccVM_SepVol(
    const c_CapOcta_TTT* a_capped_octahedron, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_SymTet_LocSepGroupLink_TagAccVM2_Vol(
    const c_SymTet* a_sym_tet, const c_LocSepGroupLink* a_localized_separator_group_link,
...
void c_getNormMoments_SymPyrmd_LocSepGroupLink_TagAccVM2_Vol(
    const c_SymPyrmd* a_sym_pyramid, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
...
void c_getNormMoments_SymTriPrism_LocSepGroupLink_TagAccVM2_Vol(
    const c_SymTriPrism* a_sym_tri_prism, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
...
void c_getNormMoments_SymHex_LocSepGroupLink_TagAccVM2_Vol(
    const c_SymHex* a_sym_hex, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
...
void c_getNormMoments_SymPyrmd_LocSepLink_TagAccVM_SepVol(
    const c_SymPyrmd* a_sym_pyramid, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_sym_pyramid != nullptr);
  assert(a_sym_pyramid->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_SymTriPrism_LocSepLink_TagAccVM_SepVol(
    const c_SymTriPrism* a_sym_tri_prism, const c_LocSepLink* a_localized_separator_link,
    c_TagAccVM_SepVol* a_moments_to_return) {
  assert(a_sym_tri_prism != nullptr);
  assert(a_sym_tri_prism->obj_ptr != nullptr);
  assert(a_localized_separator_link != nullptr);
...
void c_getNormMoments_CapDod_LLLL_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_LLLL* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
  ...
  *a_moments_to_return->obj_ptr =
      IRL::c_RuntimegetMoments<IRL::TaggedAccumulatedVolumeMoments<
          IRL::TaggedAccumulatedVolumeMoments<IRL::Volume>>>(
          *a_capped_dodecahedron->obj_ptr, *a_localized_separator_group_link->obj_ptr,
          IRL::C_CUTTING_METHOD);
}

void c_getNormMoments_CapDod_LLLT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_LLLT* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapDod_LTLT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_LTLT* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapDod_LLTT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_LLTT* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapDod_LTTT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_LTTT* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapDod_TTTT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapDod_TTTT* a_capped_dodecahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_dodecahedron != nullptr);
  assert(a_capped_dodecahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapOcta_LLL_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapOcta_LLL* a_capped_octahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapOcta_LLT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapOcta_LLT* a_capped_octahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapOcta_LTT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapOcta_LTT* a_capped_octahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_CapOcta_TTT_LocSepGroupLink_TagAccVM2_Vol(
    const c_CapOcta_TTT* a_capped_octahedron, const c_LocSepGroupLink* a_localized_separator_group_link,
    c_TagAccVM2_Vol* a_moments_to_return) {
  assert(a_capped_octahedron != nullptr);
  assert(a_capped_octahedron->obj_ptr != nullptr);
  assert(a_localized_separator_group_link != nullptr);
...
void c_getNormMoments_Tet_PlanarSepPathGroup_TagAccVM_Vol(const c_Tet* a_tet,
    const c_PlanarSepPathGroup* a_path,
	c_TagAccVM_Vol* a_moments_to_return){
  assert(a_tet != nullptr);
  assert(a_tet->obj_ptr != nullptr);
  ...
  *a_moments_to_return->obj_ptr =
      IRL::c_RuntimegetMoments<
	  IRL::TaggedAccumulatedVolumeMoments<IRL::Volume>>(
          *a_tet->obj_ptr, *a_path->obj_ptr,
          IRL::C_CUTTING_METHOD);
}

void c_getNormMoments_Pyrmd_PlanarSepPathGroup_TagAccVM_Vol(const c_Pyrmd* a_pyramid,
    const c_PlanarSepPathGroup* a_path,
	c_TagAccVM_Vol* a_moments_to_return){
  assert(a_pyramid != nullptr);
  assert(a_pyramid->obj_ptr != nullptr);
  ...
  *a_moments_to_return->obj_ptr =
      IRL::c_RuntimegetMoments<
	  IRL::TaggedAccumulatedVolumeMoments<IRL::Volume>>(
          *a_pyramid->obj_ptr, *a_path->obj_ptr,
          IRL::C_CUTTING_METHOD);
}

void c_getNormMoments_TriPrism_PlanarSepPathGroup_TagAccVM_Vol(const c_TriPrism* a_tri_prism,
    const c_PlanarSepPathGroup* a_path,
	c_TagAccVM_Vol* a_moments_to_return){
  assert(a_tri_prism != nullptr);
  assert(a_tri_prism->obj_ptr != nullptr);
  ...
  *a_moments_to_return->obj_ptr =
      IRL::c_RuntimegetMoments<
	  IRL::TaggedAccumulatedVolumeMoments<IRL::Volume>>(
          *a_tri_prism->obj_ptr, *a_path->obj_ptr,
          IRL::C_CUTTING_METHOD);
}

void c_getNormMoments_Hex_PlanarSepPathGroup_TagAccVM_Vol(const c_Hex* a_hex,
    const c_PlanarSepPathGroup* a_path,
	c_TagAccVM_Vol* a_moments_to_return){
