...
#ifndef SRC_GENERIC_CUTTING_GENERIC_CUTTING_TPP_
#define SRC_GENERIC_CUTTING_GENERIC_CUTTING_TPP_
...
enable_if_t<is_polygon<EncompassingType>::value, bool> polytopeIsValid(
  ...
  return a_polytope.getPlaneOfExistence().normal().calculateMagnitude() > 0.999;
}
...
enable_if_t<is_polygon<EncompassingType>::value, bool> polytopeIsValid(
    const EncompassingType* a_polytope) {
  return a_polytope->getPlaneOfExistence().normal().calculateMagnitude() >
         0.999;
}
...
ReturnType getVolumeMoments<
  ...
  return IRL::getVolumeMoments<ReturnType, CuttingMethod>(a_polytope, a_reconstruction.getFirstReconstruction());
}
...
inline ReturnType getVolumeMomentsProvidedStorage<
    ...
    getVolumeMomentsImplementation(SegmentedPolytopeType* a_polytope,
                                   HalfEdgePolytopeType* a_complete_polytope,
                                   const PlanarSeparatorPathGroup& a_reconstruction) {
	  return IRL::getVolumeMoments<ReturnType, CuttingMethod>(a_polytope, a_complete_polytope, a_reconstruction.getFirstReconstruction());
}
//******************************************************************* //
