...
class VertexList {
 public:
  ...
  UnsignedIndex_t size(void) const { return referenced_vertices_m.size(); }
...
class SegmentedDecomposedPolytope
 ...
 public:
  ...
  UnsignedIndex_t getNumberOfSimplicesInDecomposition(void) const {
    return simplex_decomposition_m.size();
  }
  ...
  typename VertexList<VertexStorageType, VertexType>::iterator  begin(void) noexcept { return vertex_list_m.begin(); }
  typename VertexList<VertexStorageType, VertexType>::const_iterator  begin(void) const noexcept { return this->cbegin(); }
  typename VertexList<VertexStorageType, VertexType>::const_iterator  cbegin(void) const noexcept {
    return vertex_list_m.cbegin();
  }
  typename VertexList<VertexStorageType, VertexType>::iterator  end(void) noexcept { return vertex_list_m.end(); }
  typename VertexList<VertexStorageType, VertexType>::const_iterator  end(void) const noexcept { return this->cend(); }
  typename VertexList<VertexStorageType, VertexType>::const_iterator  cend(void) const noexcept {
...
template <class VertexStorageType, class VertexType, class ProxyType>
inline std::ostream& operator<<(std::ostream& out, const SegmentedDecomposedPolytope<VertexStorageType, VertexType, ProxyType>& a_decomposed_polytope) {
  out << "Polytope object made of " << a_decomposed_polytope.getNumberOfSimplicesInDecomposition() << " simplices " << std::endl;
  out << "They are: " << std::endl;
  for(UnsignedIndex_t n = 0; n < a_decomposed_polytope.getNumberOfSimplicesInDecomposition(); ++n){
