...
void HalfEdgePolytope<
    PtType, VertexType, HalfEdgeType, FaceType, kMaxHalfEdges, kMaxVertices,
    kMaxFaces>::freeVertexAndHalfEdges(VertexType* a_vertex) {
  // Traverse HalfEdges on the vertex and remove
  auto current_half_edge = a_vertex->getHalfEdge();
  do {
    half_edges_m.freeObject(current_half_edge);    
    current_half_edge = current_half_edge->getOppositeHalfEdge()->getPreviousHalfEdge();
  } while(current_half_edge != a_vertex->getHalfEdge());
  vertices_m.freeObject(a_vertex);
}
...
void HalfEdgePolytope<
    PtType, VertexType, HalfEdgeType, FaceType, kMaxHalfEdges, kMaxVertices,
    kMaxFaces>::freeFace(FaceType* a_face) {
  faces_m.freeObject(a_face);
}
