...
#ifndef SRC_HELPERS_GEOMETRIC_CUTTING_HELPERS_TPP_
#define SRC_HELPERS_GEOMETRIC_CUTTING_HELPERS_TPP_
...
inline UnsignedIndex_t locatePt(const Pt& a_pt,
  ...
  const auto& deciding_reconstruction = a_reconstruction_link.getFirstReconstruction();
  for(UnsignedIndex_t n = 0; n < deciding_reconstruction.getNumberOfPlanes(); ++n){
 	if(deciding_reconstruction[n].signedDistanceToPoint(a_pt) > 0.0){
	  assert(a_reconstruction_link.hasNeighbor(n));
	  return locatePt(a_pt, a_reconstruction_link.getNeighbor(n));
 	}
  }
...
#endif  // SRC_HELPERS_GEOMETRIC_CUTTING_HELPERS_TPP_
