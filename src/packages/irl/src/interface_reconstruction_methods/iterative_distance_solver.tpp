...
template <class CellType, UnsignedIndex_t kMaxPlanes>
SmallVector<double, kMaxPlanes>
IterativeSolverForDistance<CellType, kMaxPlanes>::getDistances(void) {
  auto return_array = distances_m;
  if(flipped_solution_m){
    for(auto& member : return_array){
	  member = -member;
    }
  }
  return return_array;
...
double IterativeSolverForDistance<CellType, kMaxPlanes>::getDistances(
  ...
  return flipped_solution_m ? -distances_m[a_p] : distances_m[a_p];
}
...
void IterativeSolverForDistance<CellType, kMaxPlanes>::setup(void) {
  if(target_volume_fraction_m > 0.5){
	...
	if(reconstruction_m.getNumberOfPlanes() > 1){
	  ...
	  //auto tmp_plane = reconstruction_m[0];
	  //reconstruction_m[0] = reconstruction_m[1];
	  //reconstruction_m[1] = tmp_plane;
	}
  }

  // TODO checking for this initial band might actually be expensive.
  // Could potentially delay and do only if secant fails. Still
  // know the bound_error so I would know if a better bound
  // had been found during secant.
  assert(reconstruction_m.getNumberOfPlanes() <= kMaxPlanes);
  ...
  for (UnsignedIndex_t n = 0; n < reconstruction_m.getNumberOfPlanes(); ++n) {
    initial_distances_m[n] =
        clipBetween(lower_bound, reconstruction_m[n].distance(), upper_bound);
    bound_value_m[0] =
...
bool IterativeSolverForDistance<CellType, kMaxPlanes>::isBoundsTrueBounds(
  ...
  reconstruction_m.setDistances(distances_m);
  double result =
      std::fabs(getVolumeFraction<ReconstructionDefaultCuttingMethod>(
          cell_m, reconstruction_m));
  if (result > 1.0e-14) {
  ...
  reconstruction_m.setDistances(distances_m);
  result =
      std::fabs(1.0 - getVolumeFraction<ReconstructionDefaultCuttingMethod>(
                          cell_m, reconstruction_m));
  if (result > 1.0e-14) {
...
template <class CellType, UnsignedIndex_t kMaxPlanes>
void IterativeSolverForDistance<CellType, kMaxPlanes>::solveForDistance() {
  // Setup the system
  this->setup();
  ...
  secant_solver.solve(this,
                      0.05 * std::pow(cell_m.calculateVolume(), 1.0 / 3.0));
  if (secant_solver.getReason() >= 0) return;
...
#ifndef NDEBUG_PERF
  if ((initial_distances_m.capacity() >
       kMaxPlanes) ||
