...
#ifndef SRC_MOMENTS_TAGGED_ACCUMULATED_VOLUME_MOMENTS_H_
#define SRC_MOMENTS_TAGGED_ACCUMULATED_VOLUME_MOMENTS_H_
...
template <class VolumeMomentsType> class TaggedAccumulatedVolumeMoments {
  class TaggedMoments {
  public:
  ...
  static constexpr std::size_t initial_bucket_count = 50;
  using StackAllocator = short_alloc::short_alloc<
      std::pair<const UnsignedIndex_t, UnsignedIndex_t>,
      static_cast<std::size_t>(initial_bucket_count) *
...
public:
  ...
  TaggedAccumulatedVolumeMoments &
  operator=(const TaggedAccumulatedVolumeMoments &a_other) noexcept;
  ...
  getMomentsForIndex(const UnsignedIndex_t a_index) const;

  UnsignedIndex_t getTagForIndex(const UnsignedIndex_t a_index) const;

  TaggedAccumulatedVolumeMoments &
  operator+=(const TaggedAccumulatedVolumeMoments &a_other);
...
template <class VolumeMomentsType>
inline TaggedAccumulatedVolumeMoments<VolumeMomentsType>
operator*(const TaggedAccumulatedVolumeMoments<VolumeMomentsType> &a_list,
...
#include "src/moments/tagged_accumulated_volume_moments.tpp"

#endif // SRC_MOMENTS_TAGGED_ACCUMULATED_VOLUME_MOMENTS_H_
