...
TaggedAccumulatedVolumeMoments<
    ...
    : memory_pool_m(),
      tag_to_vector_index_m(initial_bucket_count, std::hash<UnsignedIndex_t>(),
                            std::equal_to<UnsignedIndex_t>(), memory_pool_m) {
...
TaggedAccumulatedVolumeMoments<VolumeMomentsType>::
    ...
    : accumulated_moments_m(a_other.accumulated_moments_m), memory_pool_m(),
      tag_to_vector_index_m(initial_bucket_count, std::hash<UnsignedIndex_t>(),
                            std::equal_to<UnsignedIndex_t>(), memory_pool_m) {
...
operator[](const UnsignedIndex_t a_tag) {
  ...
  if (insert_pair.second) { // Successful insertion
    accumulated_moments_m.emplace_back(TaggedMoments(VolumeMomentsType(),
    		a_tag));
...
operator+=(const TaggedAccumulatedVolumeMoments &a_other) {
  for (UnsignedIndex_t elem = 0; elem < a_other.size(); ++elem) {
    ...
    if (insert_pair.second) { // Successful insertion
      accumulated_moments_m.emplace_back(TaggedMoments(a_other.getMomentsForIndex(elem),
              tag_for_others_element));
...
template <class VolumeMomentsType>
UnsignedIndex_t
TaggedAccumulatedVolumeMoments<VolumeMomentsType>::size(void) const {
