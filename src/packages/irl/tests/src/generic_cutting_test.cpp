...
TEST(GenericCutting, MethodDifference){
	...
	auto cap_octa = CappedOctahedron_LLL({
		Pt( 0, 0.40625, -0.015625 ),
		Pt( 0.03125, 0.375, 0.015625 ),
		Pt( 0, 0.375, 0.015625 ),
	...
	localizer.addPlane(Plane(Normal( 0.0, 0.707106781186547, 0.707106781186547), 0.276213586400995 ));
	localizer.addPlane(Plane(Normal(-0.707106781186547, -0.0, 0.707106781186547 ),-0.0110485434560398  ));
	localizer.addPlane(Plane(Normal(0.577350269189626, -0.577350269189626, -0.577350269189626 ), -0.207485252990022 ));
	localizer.addPlane(Plane(Normal(0.0, 0.0, -1.0 ), 0.015625 ));

	PlanarSeparator separator;
	separator.addPlane(Plane(Normal(-0.207018186962485, -0.976966875955128, -0.0517609268970713),-0.392491275508266));

	LocalizedSeparator localized_separator(&localizer, &separator);


	auto simplex_moments = getVolumeMoments<SeparatedMoments<Volume>, SimplexCutting>(cap_octa, localized_separator);
	auto recursive_moments = getVolumeMoments<SeparatedMoments<Volume>, RecursiveSimplexCutting>(cap_octa, localized_separator);
