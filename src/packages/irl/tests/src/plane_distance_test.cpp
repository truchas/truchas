...
TEST(PlaneDistance, findDistanceOnePlane) {
  ...
  for (int cycle = 0; cycle < ncycles; ++cycle) {
    ...
    reconstruction[0].distance() =
        findDistanceOnePlane(unit_cell, cut_volume, reconstruction[0].normal());
    ...
    EXPECT_NEAR(new_cut_volume, cut_volume, 1.0e-14)
        ...
        << " which led to a volume of " << cut_volume << '\n';
  }
}
...
TEST(PlaneDistance, findDistanceTwoPlane) {
  ...
  for (int cycle = 0; cycle < ncycles; ++cycle) {
    ...
    IterativeSolverForDistance<RectangularCuboid> solver(
        cell, cut_volume, tolerance, reconstruction);
    reconstruction.setDistances(solver.getDistances());
    ...
    EXPECT_NEAR(new_cut_volume, cut_volume, tolerance)
        ...
        << "Returned distances : " << solver.getDistances(0) << " "
        << solver.getDistances(1) << '\n'
        << "Cell: \n"
...
TEST(PlaneDistance, findDistanceThreePlane) {
  ...
  IterativeSolverForDistance<RectangularCuboid, 3> solver(
      cell, target_volume,
  ...
  target_volume = 0.75;
  solver = IterativeSolverForDistance<RectangularCuboid, 3>(
      cell, target_volume,
      global_constants::TWO_PLANE_DISTANCE_VOLUME_FRACTION_TOLERANCE, open_box);
  open_box.setDistances(solver.getDistances());
