...
TEST(VolumeFractionMatching, setDistanceToMatchVolumeFractionPartialFill_RectangularCuboid) {
...
TEST(VolumeFractionMatching, setDistanceToMatchVolumeFractionPartialFill_Hexahedron) {

  auto hex_cell = Hexahedron({Pt(1.0, 0.0, 0.0),
	  Pt(0.75, 1.0, 0.0),
	  Pt(0.75, 1.0, 1.0),
	  Pt(1.0, 0.0, 1.0),
	  Pt(0.0, 0.0, 0.0),
...
TEST(VolumeFractionMatching, setGroupDistanceToMatchVolumeFraction) {
	...
	for(UnsignedIndex_t n = 0; n < 4; ++n){
		separators[n] = PlanarSeparator::fromOnePlane(Plane(Normal(0.0, 1.0, 0.0),0.0));
	...
	setGroupDistanceToMatchVolumeFraction(unit_cell, volume_fraction, &path_group, 1.0e-15);
	...
	setGroupDistanceToMatchVolumeFraction(unit_cell, single_phase, &path_group, 1.0e-15);
