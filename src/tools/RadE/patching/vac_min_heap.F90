...
!! the anchor vertices. It is used in the Vertex Anchor Clustering algorithm
!!
...
!!
!!  The module defines the VAC_MIN_HEAP derived type. It has the
...
contains
  ...
  subroutine put(this, node, faces, weight)

    class(min_heap), intent(inout) :: this
    integer, intent(in) :: node, faces(:)
    real(r8), intent(in) :: weight

    type(heap_entry), allocatable :: temp(:)
    integer :: n

    this%n = this%n + 1

    !! If capacity is exceeded, grow heap
    if (this%n > size(this%heap)) then
      n = size(this%heap)
      allocate(temp(2*n))
    ...
    n = this%n
    this%heap(n)%node   = node
    this%heap(n)%face   = faces
    this%heap(n)%weight = weight
