...
!! VAC_PATCHING
!!
!! This module defines derived types that encapsulate the Vertex Anchor
!! Clustering (VAC) algorithm and the related Vertex Anchor Cluster Paving
!! algorithm (PAVE) for clustering faces of a radiation enclosure mesh.
!!
...
!!
!!  VAC_PATCHING and PAVE_PATCHING share the same interface, so we only list it
!!  once. Details specific to each algorithm are discussed in the IMPLEMENTATION
!!  NOTES section below.
!!
!!   INIT(E, MAX_ANGLE, MERGE_LEVEL, SPLIT_PATCH_SIZE, VERBOSITY [,SEED])
!!     initializes the object.  E is an instance of the TYPE(ENCL) radiation
!!     enclosure.  MAX_ANGLE is a real scalar that defines the maximum allowable
!!     angle in degrees between normals of adjacent faces.  If two topologically
!!     adjacent faces are at an angle greater than MAX_ANGLE degrees, they will
!!     not be considered adjacent during patch construction.  MERGE_LEVEL is an
!!     integer scalar that controls the aggressiveness of patch merging.  A
!!     merge level of 0 indicates no merging.  A merge level >= 3 indicates the
!!     most aggressive merging method.  SPLIT_PATCH_SIZE is an integer scalar
!!     that defines the maximum size of patches that are 'split' into 1-face
!!     patches during patch merging.  Actually, we queue these 1-face patches
!!     along with the original patch.  The 1-face patches have a large weight,
!!     and are only used to 'fill-in the gaps' after merging.  VERBOSITY is a
!!     integer scalar that specifies the verbosity level of all messages printed
!!     by the object.  A verbosity level <= 0 suppresses all output.  If the
!!     optional integer argument SEED is present, it is used to initialize the
!!     random number generator used to pick the initial seed patches for the
!!     PAVE algorithm.  If SEED is not present, the seed is taken from the
!!     system clock.  Note that the VAC algorithm does not use randomness, so
!!     this argument does not affect the execution of VAC, but the random seed
!!     will still be set.
!!
!!   RUN() executes the [VAC/PAVE] patching algorithm.  At the end of this
...
!!     and must be called after the object is initialized.
!!
...
contains
  ...
  subroutine init_vac_patching(this, e, max_angle, merge_level, split_patch_size, verbosity, seed)

    use cell_topology, only: get_edge_nodes
    use cell_geometry, only: face_normal, vector_length, normalized
    use patching_tools, only: init_random_seed, get_fnhbr_aux, faces_neighboring_vertices
    ...
    if (this%verbosity > 1) then
      ...
      print '("  MERGE LEVEL:", i3)', merge_level
      print '("  SPLIT PATCH SIZE:", i3)', split_patch_size
  ...
  !! The main VAC paving algorithm
  subroutine run_pave(this)
