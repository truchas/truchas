...
!!
!!  INIT (THIS, E, AVG_FPP, MAX_ANGLE, VERBOSITY) initializes THIS.  E is an
!!    instance of the TYPE(ENCL) radiation enclosure.  AVG_FPP is a real scalar
!!    that defines the average faces per patch desired.  The average faces per
!!    patch is calculated as [# FACES]/[# PATCHES].  AVG_FPP defines the number
!!    of patch seeds with which to initialize the algorithm, and therefore the
!!    total number of patches in the output:
!!      [# SEEDS] = [# FACES] / AVG_FPP
...
!!    scalar that specifies the verbosity level of all messages printed by THIS.
!!    A verbosity level <= 0 suppresses all output.
!!
!!  RUN (THIS, MIN_DELTA, MAX_ITER) executes the VSA patching algorithm.
...
!!    regardless of other stopping conditions.  At the end of this procedure
!!    the internal data of THIS structures will hold a valid patching of the
!!    enclosure.  This procedure must be called after THIS is initialized.
!!
...
contains
  ...
  subroutine init_vsa_patching(this, e, avg_fpp, max_angle, verbosity)
