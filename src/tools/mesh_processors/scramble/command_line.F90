...
contains
  ...
  subroutine usage_summary ()
    ...
    write(unit=*,fmt='(a)') ''
    write(unit=*,fmt='(a)') 'This program generates a new mesh where each element has been replaced'
    write(unit=*,fmt='(a)') 'by a randomly-selected equivalent relabeling of the element nodes; only'
    write(unit=*,fmt='(a)') '8-node hex cells are relabeled currently.'
    write(unit=*,fmt='(a)') ''
    write(unit=*,fmt='(a)') 'The resulting mesh should be numerically equivalent to the input mesh.'
    write(unit=*,fmt='(a)') ''
