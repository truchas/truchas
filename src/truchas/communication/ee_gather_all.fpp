...
!    integer, POINTER, Dimension(:) :: Duplicate_Indx
  ...
  IF (NEW_BOUNDARY) THEN
     ALLOCATE(Supplement_Data(PGSLib_Size_Of_Sup(EE_All_Ngbr_Trace)))
     ...
     ALLOCATE(Duplicate_Data(PGSLib_Size_Of_Dup(EE_All_Ngbr_Trace)))
!     ALLOCATE(Duplicate_Indx(PGSLib_Size_Of_Dup(EE_All_Ngbr_Trace)))
     do i = 1, PGSLib_Size_Of_Dup(EE_All_Ngbr_Trace)
        Duplicate_Data(i) = SOURCE(PGSLib_Dup_Index(EE_All_Ngbr_Trace, i))
     end do
  ...
  ELSE
     ...
     IF (SIZE(Supplement_Data,1) /= PGSLib_Size_Of_Sup(EE_All_Ngbr_Trace)) THEN
  ...
  do c = lc, uc
     ...
     ! and of the Destination for cell c.
     Cell_Ngbrs => FLATTEN(Mesh(c)%Ngbr_Cells_All)
     Dest_Column => FLATTEN(Dest(i))

     do f = 1, SIZES(Dest(i))
        if (Cell_Ngbrs(f) > 0) then
           Dest_Column(f) = SOURCE(Cell_Ngbrs(f))
        else
           Dest_Column(f) = Supplement_Data( -Cell_Ngbrs(f))
        end if
