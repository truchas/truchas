             ...
             do c = 1,SIZE(Mesh,1)                  ! Loop over cells
                do f = 1,SIZE(Mesh(1)%Ngbr_cell,1)  ! Loop over faces
                   ...
                   IF ( Mesh(c)%Ngbr_cell(f) > 0) then
                      Dest(Mesh(c)%Ngbr_cell(f)) = _PREFIX_OP_(Dest(Mesh(c)%Ngbr_cell(f)), Src(f,c) )
                   ELSE
                      Supplement_Data( -Mesh(c)%Ngbr_cell(f)) = &
                           &  _PREFIX_OP_(Supplement_Data( -Mesh(c)%Ngbr_cell(f)), Src(f,c) )
                   ENDIF
                end do
             end do
          ELSE
             ...
             do c = 1,SIZE(Mesh,1)                 ! Loop over cells
                do v = 1,SIZE(Mesh(1)%Ngbr_vrtx,1) ! Loop over vertices
                   ...
                   IF ( Mesh(c)%Ngbr_vrtx(v) > 0) then
                      Dest(Mesh(c)%Ngbr_vrtx(v)) = _PREFIX_OP_(Dest(Mesh(c)%Ngbr_vrtx(v)), Src(v,c) )
                   ELSE
                      Supplement_Data( -Mesh(c)%Ngbr_vrtx(v)) = &
                           &  _PREFIX_OP_(Supplement_Data( -Mesh(c)%Ngbr_vrtx(v)), Src(v,c) )
                   ENDIF
