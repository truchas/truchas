...
contains
  ...
  subroutine test1_rand
    ...
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,0,1,0,0,0,0]))
    call run_test (mesh, grad, func, 2.7e-2_r8, 'test1-rand-quad1.gmv')

    call TLS_info ('Recovering another quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,1,0,0,1,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,1,0,0,1,0,0]))
    call run_test (mesh, grad, func, 7.0e-2_r8, 'test1-rand-quad2.gmv')
  ...
  subroutine test1_pave
    ...
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,0,1,0,0,0,0]))
    call run_test (mesh, grad, func, 4.0e-2_r8, 'test1-pave-quad1.gmv')

    call TLS_info ('Recovering another quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0]))
    call run_test (mesh, grad, func, 6.0e-2_r8, 'test1-pave-quad2.gmv')
  ...
  subroutine test2_unif
    ...
    allocate(func, source=linear_func([real(r8)::1.1,0.9,0]))
    call run_test (mesh, grad, func, 7.0e-5_r8, 'test2-unif-lin.gmv')

    call TLS_info ('Recovering quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,0,1,0,0,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,0,1,0,0,0,0]))
    call run_test (mesh, grad, func, 4.0e-2_r8, 'test2-unif-quad1.gmv')

    call TLS_info ('Recovering another quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0]))
    call run_test (mesh, grad, func, 6.0e-2_r8, 'test2-unif-quad2.gmv')
  ...
  subroutine test2_pave
    ...
    !F2008: func = linear_func([real(r8)::1.1,0.9,0])
    allocate(func, source=linear_func([real(r8)::1.1,0.9,0]))
    call run_test (mesh, grad, func, 5.0e-5_r8, 'test2-pave-lin.gmv')

    call TLS_info ('Recovering quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,0,1,0,0,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,0,1,0,0,0,0]))
    call run_test (mesh, grad, func, 6.0e-2_r8, 'test2-pave-quad1.gmv')

    call TLS_info ('Recovering another quadratic function gradient...')
    !F2008: func = quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0])
    deallocate(func)
    allocate(func, source=quad_func([real(r8)::1,2,0,1,0.5,0,-1,0,0]))
    call run_test (mesh, grad, func, 8.0e-2_r8, 'test2-pave-quad2.gmv')
  ...
  subroutine test3_unif
    ...
    call run_test (mesh, grad, func, 0.05_r8, 'test3-unif.gmv')
