...
program test_rade
  ...
  call init_state (mesh, er_params)
  ...
  !! Exterior enclosure
  call test_converge (mesh, er_params, ENCL_EXT_FACE, hc_temp)
  call test_converge (mesh, er_params, ENCL_EXT_PATCH, hc_temp)

  !! Interior enclosure
  call test_converge (mesh, er_params, ENCL_INT_FACE, hc_temp)
  call test_converge (mesh, er_params, ENCL_INT_PATCH, hc_temp)
...
contains
  ...
  subroutine init_state (mesh, er_params)
    ...
    type(parameter_list), intent(inout), target :: er_params
    ...
    call read_enclosure_radiation_namelists(lun, er_params)
    call set_solver_controls(er_params)
  ...
  subroutine set_solver_controls(er_params)

    use parameter_list_type, only: parameter_list_iterator
    ...
    do n = 1, piter%count()
      plist => piter%sublist()
      call plist%set('error-tol', 1.0e-9_r8)
      call piter%next
    end do
  ...
  subroutine test_converge (mesh, er_params, encl_name, hc_temp)
    ...
    plist => er_params%sublist(encl_name)
    call plist%get('error-tol', tol)
    call prob%init(mesh, encl_name, plist)
    call prob%set_initial_time(0.0d0)
