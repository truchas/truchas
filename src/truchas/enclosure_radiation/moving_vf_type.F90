...
module moving_vf_type
  ...
  type, extends(encl_vf), public :: moving_vf
    private
    integer :: n = 0 ! current interval index [times(n), times(n+1)]
...
contains
  ...
  subroutine init(this, times, filenames)
    ...
    integer :: n
    logical :: flag
    class(static_vf), allocatable :: vf
    ...
    !! Inherit the fixed enclosure topology from a representative file.
    !! The time-dependent view factors are loaded by set_initial_time.
    call alloc_const_vf(vf, filenames(1))
    call copy_base_components(src=vf, dest=this)
  ...
  subroutine set_initial_time(this, t)
    ...
    if (allocated(this%vf1)) deallocate(this%vf1)
    if (allocated(this%vf2)) deallocate(this%vf2)

    !! Find N such that TIMES(N) <= T < TIMES(N+1).
    do n = 0, size(this%times)-1
      if (t < this%times(n+1)) exit
    end do

    !! Load the static view factors at the interval endpoints.
    if (n > 0) call alloc_const_vf(this%vf1, this%filenames(n))
    if (n < size(this%times)) call alloc_const_vf(this%vf2, this%filenames(n+1))
    this%n = n

    !! Moving view factors require a fixed topology over the entire series.
    if (allocated(this%vf1)) then
      INSIST(compatible_vf(this, this%vf1))
    end if
    if (allocated(this%vf2)) then
      INSIST(compatible_vf(this, this%vf2))
    end if

    call set_time(this, t)
