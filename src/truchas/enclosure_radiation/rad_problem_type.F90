 ...
 !!
  ...
  subroutine init (this, mesh, name, params)
    ...
    if (params%is_parameter('toolpath')) then
      call alloc_moving_vf(params, this%vf, filename)
  ...
  subroutine set_initial_time(this, time)
    ...
    type is (moving_vf)
      call vf%set_initial_time(time)
    end select

    !! The diffusion driver calls this before moving view-factor events can
    !! advance the view-factor series.
    !!
    !! This assumes that the emissivities are time-independent so that we can
    !! compute the Chebyshev parameters once.  Really these need to be computed
    !! whenever the emissivities change significantly, but this involves
    !! computing eigenvalues of the radiosity system and isn't cheap.
    call this%sol%set_cheby_param(time=time)
  ...
  subroutine alloc_moving_vf(params, vf, filename)
  ...
  subroutine add_moving_vf_events(this, eventq, rank)
    ...
    type is (moving_vf)
      do j = 1, size(vf%times)
        call eventq%add_event(vf%times(j), vf_event(DT_POLICY_FACTOR, 0.125_r8), rank=rank)
      end do
    end select
  end subroutine
