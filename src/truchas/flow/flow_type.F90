...
contains
  ...
  subroutine set_initial_state_start(this, t, dt, vcell, vof, tcell, state)

    class(flow), intent(inout) :: this

    real(r8), intent(in) :: t, dt
    real(r8), intent(in) :: vcell(:,:), vof(:,:), tcell(:), state(:,:)
    ...
    call this%props%set_initial_state(vof, tcell, state)
  ...
  subroutine set_initial_state_restart(this, t, pcell, vcell, vface, vof, tcell, state)

    class(flow), intent(inout) :: this
    real(r8), intent(in) :: t, pcell(:), vcell(:,:), vface(:), tcell(:), state(:,:), vof(:,:)
  ...
  subroutine step(this, t, dt, vof, flux_volumes, tcell, state)

    class(flow), intent(inout) :: this
    real(r8), intent(in) :: t, dt, vof(:,:), flux_volumes(:,:), tcell(:), state(:,:)

    call this%props%update_cc(vof, tcell, state)
