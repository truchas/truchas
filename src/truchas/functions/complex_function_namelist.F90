...
!!
!! This module provides procedures for reading the COMPLEX_FUNCTION namelists.
!!
...
!!
!!  CALL READ_COMPLEX_FUNCTION_NAMELISTS(LUN) reads all the so-named namelists
!!    and creates the specified functions as COMPLEX_SCALAR_FUNC class objects,
!!    and adds them to the truchas function table.
!!

#include "f90_assert.fpp"

module complex_function_namelist

  implicit none
  private

  public :: read_complex_function_namelists

  integer, parameter :: MAX_PARAM = 16

contains

  subroutine read_complex_function_namelists(lun)
    ...
    namelist/complex_function/ name, type, library_path, library_symbol, parameters

    call TLS_info ('Reading COMPLEX_FUNCTION namelists ...')

    if (is_IOP) rewind(lun)
    n = 0

    do  ! until all COMPLEX_FUNCTION namelists have been read or an error occurs

      if (is_IOP) call seek_to_namelist(lun, 'complex_function', found, iostat=stat)
      call broadcast(stat)
      if (stat /= 0) call TLS_fatal('error reading input file: iostat=' // i_to_c(stat))
      call broadcast(found)
      if (.not.found) exit ! no further COMPLEX_FUNCTION namelists found

      n = n + 1
      label = 'COMPLEX_FUNCTION[' // i_to_c(n) // ']'
      ...
      !! Read the namelist
      if (is_IOP) read(lun, nml=complex_function, iostat=stat, iomsg=iom)
      call broadcast(stat)
      ...
      case ('library')
        ...
        call alloc_dl_complex_scalar_func(f, library_path, library_symbol, parameters(:npar))
  ...
  end subroutine read_complex_function_namelists

end module complex_function_namelist
