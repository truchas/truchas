!!
!! COMPLEX_VECTOR_FUNC_FACTORIES
!!
!! Procedures for instantiating new COMPLEX_VECTOR_FUNC class objects. All
!! a new CLASS(COMPLEX_VECTOR_FUNC) object, but the dynamic type is determined
...
module complex_vector_func_factories

  use,intrinsic :: iso_fortran_env, only: r8 => real64
  use complex_vector_func_class
  use const_complex_vector_func_type
  use dl_complex_vector_func_type
  implicit none
  private

  public :: complex_vector_func, alloc_complex_vector_func
  
  !! Export type-specific allocation procedures from the specific type modules
  public :: alloc_const_complex_vector_func, alloc_dl_complex_vector_func

  interface alloc_complex_vector_func
    procedure alloc_complex_vector_func, get_complex_vector_func
  end interface

contains

  !TODO: this needs error handling
  subroutine alloc_complex_vector_func(f, params)
  ...
  end subroutine alloc_complex_vector_func

  !! This subroutine gets the complex vector function specified by the value of
  ...
  !! vector function in the complex vector function table, or a parameter list
  !! that defines the function.

  subroutine get_complex_vector_func(plist, param, f, stat, errmsg)
    ...
    if (plist%is_sublist(param)) then
      ...
      call alloc_complex_vector_func(f, func_params)  !TODO: should return stat, errmsg
  ...
  end subroutine get_complex_vector_func

end module complex_vector_func_factories
