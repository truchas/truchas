...
module dl_complex_vector_func_type
  ...
  public :: alloc_dl_complex_vector_func

  type, extends(complex_vector_func), public :: dl_complex_vector_func
    ! private
  ...
  contains
    procedure :: eval
    final :: dl_complex_vector_func_delete
  end type
  ...
  !! Defined constructor
  interface dl_complex_vector_func
    procedure dl_complex_vector_func_value
  end interface

contains

  subroutine alloc_dl_complex_vector_func(f, lib, sym, dim, p)
    class(complex_vector_func), allocatable, intent(out) :: f
    character(*), intent(in) :: lib, sym
    integer, intent(in) :: dim
    real(r8), intent(in), optional :: p(:)
    allocate(f, source=dl_complex_vector_func(lib, sym, dim, p))
  end subroutine

  !! Final subroutine for DL_COMPLEX_VECTOR_FUNC objects
  subroutine dl_complex_vector_func_delete(this)
    type(dl_complex_vector_func), intent(inout) :: this
    !call this%so%close() NNC: this does not play nice with assignment -- temporary fix
    this%f => null()
  end subroutine

  !! Constructor for DL_COMPLEX_VECTOR_FUNC objects
  function dl_complex_vector_func_value(lib, sym, dim, p) result(dlf)
    ...
    integer, intent(in) :: dim
    real(r8), intent(in), optional :: p(:)
    type(dl_complex_vector_func) :: dlf
  ...
  end function dl_complex_vector_func_value

  function eval(this, x) result(fx)
    class(dl_complex_vector_func), intent(in) :: this
    real(r8), intent(in) :: x(:)
    complex(r8) :: fx(this%dim)
    call this%f(x, this%p, fx)
  end function

end module dl_complex_vector_func_type
