...
!! provides some procedures for writing the original mesh (from old_mesh_api)
...
contains
  ...
  subroutine gmv_write_mesh
    ...
    use mesh_parameter_module, only: ncells_tot, nnodes_tot, ncells
    use mesh_module, only: mesh, vertex, unpermute_mesh_vector, unpermute_vertex_vector
    use string_utilities, only: i_to_c

    integer,  allocatable :: cnode(:,:), pdata(:), map(:)
  ...
  subroutine gmv_write_cell_var (u, name)
    ...
    use mesh_parameter_module, only: ncells_tot
  ...
  subroutine gmv_write_node_var (u, name)

    use mesh_parameter_module, only: nnodes_tot
    ...
    allocate(u_global(merge(nnodes_tot,0,is_IOP)))
    call collate (u_global, u)
    if (is_IOP) call gmvwrite_variable_name_data_f (NODEDATA, name, u_global)
    deallocate(u_global)

  end subroutine gmv_write_node_var

end module orig_old_mesh_gmv
