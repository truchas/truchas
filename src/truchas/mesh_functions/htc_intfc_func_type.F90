...
!! HTC_INTFC_FUNC_TYPE
!!
!! This module defines a heat-transfer-coefficient interface flux function on a
!! subset of mesh interface face pairs. The function stores the sparse
!! face-pair set and evaluates user-supplied HTC scalar functions for thermal
!! transport models.
!!
...
!! Notes
!!
!! Each value is the heat flux leaving INDEX(1,J) and entering INDEX(2,J). The
!! HTC coefficient is evaluated using the maximum temperature across the two
!! faces. The user-function arguments are (max(T(face1),T(face2)),time,x,y,z).
!!
!! COMPUTE_DERIV returns derivatives with respect to the temperature on each
!! face. These derivatives include a centered finite-difference approximation
!! of the HTC coefficient's dependence on temperature.
!!
...
module htc_intfc_func_type
  ...
  type, extends(intfc_field_func), public :: htc_intfc_func
    private
...
contains
  ...
  subroutine compute_value(this, t, u, value)
    ...
    integer :: n, j
    real(r8) :: args(-1:size(this%mesh%x,dim=1)), c
