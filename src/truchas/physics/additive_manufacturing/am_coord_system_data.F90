...
module am_coord_system_data
  ...
  real(r8) :: nozzle_x, nozzle_y, nozzle_z, nozzle_speed, min_x, max_x, &
              coordinate_scale_factor, direction_x, direction_y, direction_z
...
contains
  ...
  subroutine read_am_coord_system_block_namelist (lun)
    ...
    namelist /am_coord_system/ nozzle_x, nozzle_y, &
                               ...
                               coordinate_scale_factor, direction_x, &
  ...
  !  INSIST(am_enabled)
    ...
    call broadcast (direction_x)
    call broadcast (direction_y)
    call broadcast (direction_z)
    ...
    if (direction_x  == NULL_R .or. direction_y  == NULL_R .or. &
        direction_z  == NULL_R) then
         direction_x = 1.0_r8
         direction_y = 0.0_r8
         direction_z = 0.0_r8
         call TLS_info ('  using default nozzle unit velocity: (1.0, 0.0, 0.0)' )
    end if
  ...
  function plist_am_coord_system_block(nozzle_x, nozzle_y, &
    ...
    real(r8), intent(in) :: nozzle_x, nozzle_y, nozzle_z, &
                            nozzle_speed, min_x, max_x, direction_x, &
                            direction_y, direction_z
    ...
    call plist_am_coord_system_block%set ('direction_x', direction_x)
    call plist_am_coord_system_block%set ('direction_y', direction_y)
    call plist_am_coord_system_block%set ('direction_z', direction_z)
