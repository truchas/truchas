  ...
  contains
    ...
    subroutine compute_temp_from_enthalpy_density(this, n, H, T)
      ...
      if ( this%root_solver%is_bracketed ) then
        ...
        print *, "compute_temp: couldn't bracket root: "
        print *, "compute_temp: Tmin, Tmax, H, T_old are: ", Tmin, Tmax, H, this%temps(n)
        print *, "compute_temp: vof void is: ", this%void_vol_fracs(n)
      end if

    !  error = residual_for_temp(T, params)
      if ( .not. this%root_solver%found_root  ) then
        print *, "compute_temp: couldn't find root"
      else
    ...
    subroutine fixup_energy_in_nearly_void_cells(this, is_nearly_void)
      ...
      do n = 1, ncells
       ...
       !   this%ignore_cell(n) = .true.
          ...
          print *, 'fixup_energy_in_nearly_void_cells: cannot fix cell energy; pruning cell'
          print *, "neighboring temps, : ", temps_nghbrs(:,n)
          print *, "vofs, : ", void_vol_fracs_nghbrs(:,n)
          this%vol_fracs(:, n) = 0
          this%vol_fracs(this%void_index, n) = 1
       !   this%temps(n) = 0
