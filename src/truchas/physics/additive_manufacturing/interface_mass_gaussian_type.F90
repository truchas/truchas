...
module interface_mass_gaussian_type
  ...
  !    call params%get('vof_cutoff', this%vof_cutoff)  
    ...
    subroutine deposit_mass_gaussian2(this, state, am_geom, dt)
      ...
      !  state%vol_fracs(m_fluid,n) = min(state%vol_fracs(m_fluid,n), 1.0_r8)
      !  state%vol_fracs(m_void,n) = max(state%vol_fracs(m_void,n) - delta_vof, 0.0_r8)
        !!!! REMOVE, UNCOMMENT ABOVE !!!!
    ...
    end subroutine deposit_mass_gaussian2
    ...
    subroutine deposit_mass_gaussian(this, state, am_geom, dt)
      ...
      if (status /= 0) call TLS_fatal ('deposit_mass_gaussian: allocation failed')
      ...
      do n = 1, state%ncells
       if ( .not. this%interface_cells(n) ) cycle
        ...
        ! Convert this to a volume fraction, and multiply by fraction
        ! of powder that sticks to the cell
        ! To do so, need to multiply by (cell volume)/(surface area), 
        ! where (surface area) denotes
        ...
        do f = 1, nfc
          proj_onto_up_vec(f) = &
              sum( Cell(n)%Face_Normal(:,f) * upward_vec(:) )
        end do
        f_up(1:1) = maxloc( proj_onto_up_vec(1:nfc) )
        A = Cell(n)%Face_Area(f_up(1))
        V = Cell(n)%Volume
        ! amount of fluid mass being fluxed into cell---need to divide by density of fluid
    ...
    function mass_source_gaussian(this, dx, dy, dt) result(q)
      ...
      q = dt * (2*this%rate) * exp(-r) / (PI*two_sigma2)
      if (q < 1d-12) then
        q = 0
      end if
    ...
    subroutine find_and_init_interface_cells(this, state)
      ...
      do n = 1, ncells
        ...
        !  if ( (1 - state%void_vol_fracs(n)-fluid_vol_tot) > 0.0_r8) then
        !    this%powder_absorp(n) = 0
        !  else
        !    this%powder_absorp(n) = 1
        !  end if
          this%powder_absorp(n) = fluid_vol_tot/(1 - state%void_vol_fracs(n))
          !!!!!!!! REMOVE !!!!!!!
        !  this%powder_absorp(n) = 1
          !!!!!!!! REMOVE !!!!!!!
        end if
