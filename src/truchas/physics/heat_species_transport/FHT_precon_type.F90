    ...
    if (associated(this%model%vf_rad_prob)) then
      ...
      do index = 1, size(this%model%vf_rad_prob)
        if (this%vfr_precon_coupling(index) == VFR_FGS .or. &
          ...
          allocate(z(size(fq)))
          z = fq
          call this%model%vf_rad_prob(index)%precon (t, Tface(this%model%vf_rad_prob(index)%faces), z)
          if (this%vfr_precon_coupling(index) == VFR_FGS) fq = z
          !! Update the heat equation face residual.
          call this%model%vf_rad_prob(index)%precon_matvec1 (t, Tface(this%model%vf_rad_prob(index)%faces), z)
          do j = 1, size(z)
    ...
    if (associated(this%model%vf_rad_prob)) then
      ...
      do index = 1, size(this%model%vf_rad_prob)
        if (this%vfr_precon_coupling(index) == VFR_JAC .or. &
          ...
          call this%model%vf_rad_prob(index)%precon (t, Tface(this%model%vf_rad_prob(index)%faces), fq)
        end if
      end do
      call stop_timer ('VF rad precon')
    end if
