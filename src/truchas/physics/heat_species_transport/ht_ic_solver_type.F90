...
module ht_ic_solver_type
  ...
  type, public :: ht_ic_solver
    ...
    type(ht_model), pointer :: model => null()  ! reference only -- do not own
...
contains
  ...
  subroutine compute(this, t, temp, u, udot)
    ...
    if (associated(this%model%void_face)) &
        where (this%model%void_face) u%tf = this%model%void_temp
    ...
    if (associated(this%model%void_cell)) &
        where (this%model%void_cell) u%hc = 0.0_r8
  ...
  subroutine compute_udot (this, t, u, udot)
    ...
    if (associated(this%model%void_cell)) &
        where (this%model%void_cell) udot%hc = 0.0_r8
    ...
    subroutine make_matrix (matrix)
      ...
      if (associated(this%vf_rad_prob)) then
        do index = 1, size(this%vf_rad_prob)
          associate (faces => this%vf_rad_prob(index)%faces)
            allocate(values(size(faces)))
            call this%vf_rad_prob(index)%rhs_deriv (t, u%tf(faces), values)
            where (.not.this%vf_rad_prob(index)%fmask) values = 0
