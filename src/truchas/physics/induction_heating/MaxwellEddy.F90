...
contains
  ...
  subroutine step (sys, t, e, b, status, set_bv, bndry_src)
    ...
    call sys%mesh%edge_ip%gather_offp(de)
...
!!
  ...
  subroutine initialize_system (sys, mesh, eps, mu, sigma, etasq, delta, dt, emask)
    ...
    call sys%mesh%node_ip%gather_offp(sys%w0mask)
  ...
  subroutine hiptmair (sys, f, u)
    ...
    call sys%mesh%edge_ip%scatter_offp_sum(u)
    call sys%mesh%edge_ip%gather_offp(u)
  ...
  subroutine cg_ax (x, y)
    ...
    call cg_sys%mesh%edge_ip%gather_offp(y)
  ...
  subroutine hiptmair_is_symmetric (sys)
    ...
    do jproc = 1, nPE
      do j = 1, bsize_vector(jproc)
        ...
        call sys%mesh%edge_ip%gather_offp(ej)
      ...
      do j = 1, bsize_vector(jproc)
        ...
        do kproc = jproc, nPE
          ...
          do k = kstart, bsize_vector(kproc)
            ...
            call sys%mesh%edge_ip%gather_offp(ek)
