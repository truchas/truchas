!!
!! INDUCTION_HEAT_SOLVER_TYPE
!!
!! This module defines a derived type that drives the evaluation of the
!! time-varying EM heat source (primarily Joule heat) for induction heating
!! simulations. Time dependence is due to stepwise changes in the induction
!! coil currents and/or frequency, and changes in temperature-dependent EM
!! properties. When necessary, an external solver for the time harmonic
!! Maxwell equations is invoked to directly compute the EM heat source, but
!! this is avoided when possible by scaling a previously computed source,
!! zeroing the source, or leaving the source unchanged. Additional methods
!! handle writing/reading restart data.
!!
...
module induction_heat_solver_type
  ...
  type, public :: induction_heat_solver
    ...
    real(r8) :: prop_change_threshold, prev_q_scf = 0.0_r8
    real(r8), allocatable :: q(:), q_eps(:), q_mu(:), q_sigma(:), q_data(:)
...
contains
  ...
  subroutine init(this, mesh, params, stat, errmsg)
    ...
    call params%get('use-fd-solver', this%use_fd_solver, stat, errmsg, default=.false.)
    if (stat /= 0) return

  end subroutine init

  !! Return a list of times when the frequency and/or induction coil currents
  !! change abruptly (and hence also the EM heat source.) This info can be used
  !! to control time stepping to hit those times exactly and reduce the step
  !! size and restart the HT ODE integrator in response to the discontinuous
  !! heat source.

  subroutine get_event_times(this, times)
    class(induction_heat_solver), intent(in) :: this
    real(r8), allocatable, intent(out) :: times(:)
    times = this%hfield_fac%times
  end subroutine
  ...
  subroutine update_em_heat(this, t, eps, epsi, mu, sigma, q, stat, errmsg)
    ...
    if (allocated(this%q)) then ! there is an existing Q that may be usable
      ...
      else if (this%em_prop_differ(eps, mu, sigma)) then
        task = COMPUTE
      else if (this%hfield_fac%hfield_differs(this%q_data, t)) then
        task = merge(SCALE, COMPUTE, this%hfield_fac%hfield_is_scaled(this%q_data, t, scf))
      ...
      task = merge(ZERO, COMPUTE, this%hfield_fac%hfield_is_zero(t))
    end if
    ...
    case (ZERO)
      ...
      call tls_info('')
      call tls_info('INDUCTION_HEAT_SOLVER: no induction field; EM heat set to zero')
    ...
    case (SCALE)
      ...
      call tls_info('')
      call tls_info('INDUCTION_HEAT_SOLVER: induction field scaled; EM heat scaled accordingly')
    ...
    case (COMPUTE)
      ...
      comp: block
        ...
        call tls_info('')
        call tls_info('INDUCTION_HEAT_SOLVER: computing the EM heat')

        this%sim_num = this%sim_num + 1

        if (this%use_fd_solver) then
          call this%params%set('graphics-file', trim(output_dir)//'fdme-'//i_to_c(this%sim_num)//'.vtkhdf')
          call compute_joule_heat_fdme(this%mesh, freq, eps, epsi, mu, sigma, this%params, q, stat, errmsg)
        else
          call this%params%set('graphics-file', trim(output_dir)//'tdme-'//i_to_c(this%sim_num)//'.vtkhdf')
          call compute_joule_heat_tdme(this%mesh, freq, eps, mu, sigma, this%params, q, stat, errmsg)
        end if
        if (stat /= 0) exit comp
  ...
  !! This auxiliary subroutine computes the Joule heat by solving the time
  !! domain Maxwell equations over several cycles of the external magnetic
  !! source field until a periodic steady state is achieved, and calculating
  !! the time-averaged Joule heat over the final cycle.

  subroutine compute_joule_heat_tdme(mesh, freq, eps, mu, sigma, params, q, stat, errmsg)
  ...
  !! This auxiliary subroutine computes the Joule heat using the frequency
  !! domain EM solver.

  subroutine compute_joule_heat_fdme(mesh, freq, eps, epsi, mu, sigma, params, q, stat, errmsg)
  ...
  !! This auxiliary function returns true if the EM properties (conductivity
  ...
  logical function em_prop_differ(this, eps, mu, sigma) result(differ)
    ...
    if (.not.this%const_sigma .or. this%check_restart_q_props) then
      dsigma = 0.0_r8
      do j = 1, size(sigma)
    ...
    if (this%check_restart_q_props) then
      ...
      differ = (max(dmu, dsigma) > this%prop_change_threshold)
    end if
  ...
  subroutine write_restart_data(this, sim)
    ...
    call gather(this%q_sigma(:this%mesh%ncell_onP), array)
    if (is_IOP) call reorder(array, cell_perm, forward=.true.)
    call sim%write_repl_data('SIGMA', array)
  ...
  subroutine read_restart_data(this, unit, version)
    ...
    call read_var(unit, n, 'INDUCTION_HEAT_SOLVER: error reading NQ restart record')
    if (n /= this%mesh%cell_imap%global_size) &
        call halt('INDUCTION_HEAT_SOLVER: incompatible restart NQ value: ' // i_to_c(n))
    allocate(this%q(this%mesh%ncell_onp))
    call read_dist_array(unit, this%q, this%mesh%xcell(:this%mesh%ncell_onP), &
        'INDUCTION_HEAT_SOLVER: error reading Q restart record')
