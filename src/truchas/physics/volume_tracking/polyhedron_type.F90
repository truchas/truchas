...
module polyhedron_type
  ...
  type, public :: polyhedron
    ...
    real(r8)              :: vol,x0(3),xl(3)        ! should be private, but need inheritance
...
contains
  ...
  subroutine update_face_normals (this, force)
    ...
    do f = 1,this%nFaces
      if (all(near_zero(this%face_normal(:,f)))) then
        nV = count(this%face_vid(:,f)/=0)
        v = 3
        do while (all(near_zero (this%face_normal(:,f))) .and. v<=nV)
        ...
        if (v>nV .and. all(near_zero(this%face_normal(:,f)))) then
          call this%print_data ()
          call TLS_fatal ("could not calculate polyhedron face normal")
        end if
      end if
    end do
  ...
  real(r8) function volume (this)
    ...
    if (this%vol < 0.0_r8) then
      if (near_zero(this%vol, 1e5_r8*epsilon(1.0_r8))) then
    ...
    if (.not.any(side>0)) then
      ...
      if (intpoly%nVerts > 2) then
        ...
        if (ierr /= 0) then
          ...
          return
          call TLS_fatal ("polyhedron split failed")
        end if
      else
        volume_behind_plane = 0.0_r8
        return
      ...
      do v = 1,this%nVerts
        if (side(v)==0) then
          ...
          do iv = 1,intpoly%nVerts
            !if (near_zero(norm2(this%x(:,v) - intpoly%x(:,iv)))) then
