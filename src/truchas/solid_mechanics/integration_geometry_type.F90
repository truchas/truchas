...
!!
!!   nppar - a logical array storing the relative node-IP orientations:
!!       nppar(k,j) is true when IP k of node j is inward oriented with respect
!!       to node j, and false when it is outward oriented. The shape of nnpar is
!!       [256,nnode].
!!
...
module integration_geometry_type
  ...
  type, public :: integration_geometry
    ...
    integer :: nlnode
    integer, allocatable :: lnode(:,:), link_setid(:)
  ...
  interface add_element
    module procedure add_element_1, add_element_2
  end interface add_element

  interface trim_array
    module procedure trim_array_1, trim_array_2
  end interface trim_array

  interface extend_array
    module procedure extend_array_1, extend_array_2
...
contains
  ...
  subroutine compute_connectivity(this)
    ...
    allocate(this%npoint(this%xnpoint(this%mesh%nnode_onP+1)-1), &
        this%xpxn(this%xnpoint(this%mesh%nnode_onP+1)-1), this%nppar(256,this%mesh%nnode_onP))
    this%npoint = -1
    this%nppar = .false.
    k = 0
    do j = 1, this%mesh%ncell
      associate (cn => this%mesh%cnode(this%mesh%xcnode(j):this%mesh%xcnode(j+1)-1))
        ...
        do p = this%xcpoint(j), this%xcpoint(j+1)-1
          ...
          if (node2 <= this%mesh%nnode_onP) then
            ! If the below assertion fails, it means there are more
            ! cells touching some node than anticipated. It can be
            ! resolved by changing the size of the first dimension of
            ! nppar, or by moving to a ragged-array approach.
            ASSERT(k(node2)+1 > 0 .and. k(node2)+1 <= size(this%nppar,dim=1))

            ! By convention, the normal is oriented toward the 2nd node
            ! of the associated edge.
            this%nppar(k(node2)+1,node2) = .true.
  ...
  subroutine compute_link_nodes(this)
    ...
    do l = 1, this%mesh%nlink
      ...
      do n = 1, size(lnode,dim=2)
        if (unique_element(this%lnode, count, lnode(:,n))) then
          count = count + 1
          call add_element(this%lnode, count, lnode(:,n))
          call add_element(this%link_setid, count, setid)
        end if
      end do
    end do
    this%nlnode = count
    call trim_array(this%lnode, this%nlnode)

  contains

    integer function link_set_id(l, mesh)
      ...
      link_set_id = -1
      do k = 1, size(mesh%link_set_id)
        if (btest(mesh%link_set_mask(l), k)) then
          link_set_id = mesh%link_set_id(k)
          exit
        end if
      end do

    end function link_set_id
  ...
  logical function unique_element(list, len, item)
    integer, intent(in) :: list(:,:)
    integer, intent(in) :: len, item(:)
    integer :: j
    unique_element = .false.
    do j = 1, len
      if (all(item == list(:,j))) return
    end do
    unique_element = .true.
  end function unique_element
  ...
  ! Add the item to the end of the array, extending if necessary.
  ! Also sort the item as added, so it can be compared more easily.
  subroutine add_element_2(list, i, item)
    ...
    if (item(1) < item(2)) then
      ...
      list(:,i) = [item(2), item(1)]
    end if
  end subroutine add_element_2

  subroutine trim_array_1(list, len)
    integer, intent(inout), allocatable :: list(:)
    integer, intent(in) :: len
    integer, allocatable :: list_(:)
    list_ = list(:len)
    deallocate(list)
    call move_alloc(list_, list)
  end subroutine trim_array_1

  subroutine trim_array_2(list, len)
    integer, intent(inout), allocatable :: list(:,:)
    integer, intent(in) :: len
    integer, allocatable :: list_(:,:)
    list_ = list(:,:len)
    deallocate(list)
    call move_alloc(list_, list)
  end subroutine trim_array_2

  subroutine extend_array_1(list)
    integer, intent(inout), allocatable :: list(:)
    integer, allocatable :: list_(:)
    allocate(list_(2*size(list,dim=1)))
    list_(:size(list,dim=1)) = list
    deallocate(list)
    call move_alloc(list_, list)
  end subroutine extend_array_1

  subroutine extend_array_2(list)
    integer, intent(inout), allocatable :: list(:,:)
    integer, allocatable :: list_(:,:)
    allocate(list_(size(list,dim=1),2*size(list,dim=2)))
    list_(:,:size(list,dim=2)) = list
    deallocate(list)
    call move_alloc(list_, list)
  end subroutine extend_array_2
