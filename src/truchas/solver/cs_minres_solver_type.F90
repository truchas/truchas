...
module cs_minres_solver_type
  ...
  contains
    procedure :: init
    procedure :: solve
    procedure, private :: global_dotc
    procedure, private :: global_nrm2
  end type
...
contains
  ...
  subroutine init(this, n, params, comm)
...
!!!%
  ...
  subroutine solve(this, lin_op, b, x)
    ...
    complex(r8), intent(in), contiguous  :: b(:)
    complex(r8), intent(out), contiguous :: x(:)
...
!!!  end
  ...
  !! Global Euclidean norm of a complex vector (parallel version of BLAS dznrm2)

  real(r8) function global_nrm2(this, x) result(nrm2)

    class(cs_minres_solver), intent(in) :: this
    complex(r8), intent(in), contiguous :: x(:)
    ...
    nrm2 = dznrm2(this%n, x, 1)
    if (this%nproc > 1) then
      call MPI_Allreduce(nrm2**2, tmp, 1, MPI_REAL8, MPI_SUM, this%comm, ierr)
      nrm2 = sqrt(tmp)
    end if

  end function global_nrm2

  !! Global dot product of complex vectors (parallel version of BLAS zdotc)

  complex(r8) function global_dotc(this, x, y) result(dotc)

    class(cs_minres_solver), intent(in) :: this
    complex(r8), intent(in), contiguous :: x(:), y(:)

    interface
      function zdotc(n, x, incx, y, incy)
        import r8
        integer :: n, incx, incy
        complex(r8) :: x(*), y(*), zdotc
      end function
    end interface
    ...
    dotc = zdotc(this%n, x, 1, y, 1)
    if (this%nproc > 1) then
      call MPI_Allreduce(dotc, tmp, 1, MPI_COMPLEX16, MPI_SUM, this%comm, ierr)
      dotc = tmp
    end if
