...
module csr_precon_boomer_type
  ...
  type, public :: csr_precon_boomer
    private
    type(csr_matrix), pointer :: A => null() ! unowned reference
    integer :: nrows = 0
  ...
  contains
    procedure :: init
    procedure :: compute
    generic :: apply => apply1, apply2
    procedure, private :: apply1, apply2
    final :: csr_precon_boomer_delete
  end type

contains

  !! Final subroutine for PCSR_PRECON_BOOMER objects.
  subroutine csr_precon_boomer_delete(this)
    type(csr_precon_boomer), intent(inout) :: this
    integer :: ierr
    ierr = 0
  ...
  subroutine init(this, A, params, stat, errmsg)
    ...
    class(csr_precon_boomer), intent(out) :: this
    ...
    call fHYPRE_IJVectorCreate(MPI_COMM_SELF, 1, this%nrows, this%bh, ierr)
    call fHYPRE_IJVectorSetMaxOffProcElmts(this%bh, 0, ierr) ! necessary?
    INSIST(ierr == 0)

    call fHYPRE_IJVectorCreate(MPI_COMM_SELF, 1, this%nrows, this%xh, ierr)
    call fHYPRE_IJVectorSetMaxOffProcElmts(this%xh, 0, ierr) ! necessary?
    INSIST(ierr == 0)
...
!    end if
  ...
  subroutine compute(this, A)

    use hypre_utilities, only: copy_to_ijmatrix

    class(csr_precon_boomer), intent(inout) :: this
    type(csr_matrix), intent(in) :: A

    integer :: ierr

    call start_timer('hypre-matrix-copy')
    call copy_to_ijmatrix(A, this%Ah)
...
!    call fHYPRE_BoomerAMGSetStrongThreshold (this%solver, this%strong_threshold, ierr)
  ...
  subroutine apply1(this, x, y)

    class(csr_precon_boomer), intent(in) :: this
    real(r8), intent(in) :: x(:)
    ...
    ASSERT(size(y) == this%nrows)

    call start_timer('boomer-solve')

    call fHYPRE_ClearAllErrors

    rows = [(i, i=1, this%nrows)]
    ...
    call fHYPRE_IJVectorSetValues  (this%xh, this%nrows, rows, y, ierr)
    ...
    call fHYPRE_IJVectorGetValues(this%xh, this%nrows, rows, y, ierr)
    INSIST(ierr == 0)

    call stop_timer('boomer-solve')

  end subroutine apply1

  subroutine apply2(this, x)
    class(csr_precon_boomer), intent(in) :: this
    real(r8), intent(inout) :: x(:)
    integer :: i, ierr, rows(this%nrows)
    call apply1(this, x, x)
  end subroutine

end module csr_precon_boomer_type
