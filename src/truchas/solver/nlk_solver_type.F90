...
module nlk_solver_type
  ...
  type, public :: nlk_solver
    ...
    type(nka_vec), allocatable :: nka   ! nonlinear Krylov accelerator
    class(vector), allocatable :: du
  ...
  type, abstract, public :: nlk_solver_model
    class(vector), allocatable :: rhs ! made accessible to solvers
  contains
  ...
  abstract interface
    subroutine compute_f(this, u, f, ax)
      import nlk_solver_model, vector
    ...
    function du_norm(this, u, du)
      import :: nlk_solver_model, vector, r8
      class(nlk_solver_model) :: this
...
contains
  ...
  subroutine init(this, model, params, stat, errmsg)
    ...
    if (maxv > 0) then
      allocate(this%nka)
      call this%nka%init(this%du, maxv)
  ...
  subroutine solve(this, u, errc)
    ...
    do itr = 1, this%mitr
      call this%model%compute_f(u, this%du)
      lnormi = lnorm(this%du)
      call this%model%apply_precon(u, this%du) !TODO: why u as input?
      if (allocated(this%nka)) call this%nka%accel_update(this%du)
      !u = u + du
      call u%update(1.0_r8, this%du) !FIXME? Why is this "+" -- it ought to be "-" unless precon is doing something weird

      !! Error estimate.
      error = this%model%du_norm(u, this%du)
      ...
      if (this%model%is_converged(itr, u, this%du, lnormi, this%ntol)) exit
    end do
