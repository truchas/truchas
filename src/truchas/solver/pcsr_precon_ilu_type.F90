...
contains
  ...
  subroutine init(this, A, params, stat, errmsg)
    ...
    !! When used as a preconditioner Hypre recommends using 1 iteration
    call params%get('max-iter', this%max_iter, stat, errmsg, default=1)
    if (stat /= 0) then
    ...
    !! Hypre's default local reordering is 1, RCM
    call params%get('reorder-type', this%reorder_type, stat, errmsg, default=1)
    if (stat /= 0) then
      errmsg = context // errmsg
      return
    end if
  ...
  subroutine compute(this)
    ...
    !call fHYPRE_ILUSetDebugFlag(this%solver, this%debug_level, ierr)
    !call fHYPRE_ILUSetLogging(this%solver, this%logging_level, ierr)
    !INSIST(ierr == 0)

    !! Set the ILU parameters for ILU(0) as a preconditioner
    !call fHYPRE_ILUSetType(this%solver, 0, ierr) ! default, block Jacobi ILU(0)
    !call fHYPRE_ILUSetLevelOfFill(this%solver, 0, ierr) ! default fill for ILU(k)
    call fHYPRE_ILUSetTol(this%solver, 0.0_r8, ierr) ! use as preconditioner
    !call fHYPRE_ILUSetMaxIter(this%solver, this%max_iter, ierr)
    call fHYPRE_ILUSetMaxIter(this%solver, 1, ierr)
    !call fHYPRE_ILUSetLocalReordering(this%solver, this%reorder_type, ierr)
    call fHYPRE_ILUSetLocalReordering(this%solver, 1, ierr)
