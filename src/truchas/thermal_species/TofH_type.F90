...
!!    the inverse of the given property cell function HOFT, which is assumed
...
!!
!!  COMPUTE (CELL, CONC, H, TMIN, TMAX, T) does the same calculation with
!!    concentration values CONC held fixed while inverting H(T,C).
!!
!!    (Note that the mixture of materials can vary from cell to cell.)
!!    The computation involves finding the root of the function H - H(T), or
!!    H - H(T,C) when concentrations are supplied.
...
module TofH_type
  ...
  type, extends(ridders), public :: TofH
    private
    class(cell_prop_func), pointer :: HofT => null() ! unowned reference
    real(r8) :: H
    integer  :: cell
  ...
  contains
    ...
    procedure, private :: compute_tc
    procedure, private :: solve
    generic :: compute => compute_t, compute_tc
...
contains
  ...
  function f (this, x) result (fx)
    ...
    if (allocated(this%conc)) then
      call this%HofT%compute_value(this%cell, x, this%conc, fx)
    else
  ...
  subroutine init (this, HofT, eps, max_try, delta)
    ...
    class(cell_prop_func), target :: HofT
  ...
  subroutine compute_t (this, cell, H, Tmin, Tmax, T)
    ...
    this%cell = cell
    this%H = H
    call this%solve(Tmin, Tmax, T)
  end subroutine compute_t

  subroutine compute_tc (this, cell, conc, H, Tmin, Tmax, T)
    class(TofH), intent(inout) :: this
    integer,  intent(in)  :: cell
    ...
    this%H = H
    call this%solve(Tmin, Tmax, T)
  end subroutine compute_tc

  subroutine solve (this, Tmin, Tmax, T)
    use truchas_logging_services, only: TLS_fatal
    class(TofH), intent(inout) :: this
    real(r8), intent(in)  :: Tmin, Tmax
    real(r8), intent(out) :: T
    integer :: n, stat
