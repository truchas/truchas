...
contains
  ...
  subroutine read_diffusion_solver_namelist(lun, ds_sys_type, num_species, params)
    ...
    real(r8) :: residual_atol, residual_rtol, vfr_residual_tol, vfr_solve_tol

    real(r8) :: cond_vfrac_threshold, cutvof
    integer :: vfr_precon_iter
    character(len=32) :: nlk_preconditioner, stepping_method
    character(len=32) :: vfr_precon_coupling_method, vfr_precon_method
    ...
    namelist /diffusion_solver/ max_step_tries, abs_conc_tol, rel_conc_tol, &
        ...
        cond_vfrac_threshold, residual_atol, residual_rtol, vfr_residual_tol, vfr_solve_tol, &
        void_temperature, cutvof, use_backward_euler, &
        vfr_precon_coupling_method, vfr_precon_method, vfr_precon_iter
    ...
    vfr_precon_coupling_method = NULL_C
    vfr_precon_method = NULL_C
    vfr_precon_iter = NULL_I
    pc_freq = NULL_I
    ...
    vfr_residual_tol = NULL_R
    vfr_solve_tol = NULL_R
    ...
    call broadcast(vfr_precon_coupling_method)
    call broadcast(vfr_precon_method)
    call broadcast(vfr_precon_iter)
    ...
    call broadcast(vfr_residual_tol)
    call broadcast(vfr_solve_tol)
    ...
    if (vfr_residual_tol == NULL_R) then
      vfr_residual_tol = 1.0e-3_r8
    else if (vfr_residual_tol <= 0.0_r8) then
      call TLS_fatal ('VFR_RESIDUAL_TOL must be > 0')
    end if
    plist => params%sublist('norm')
    call plist%set('rad-tol', vfr_residual_tol)

    if (vfr_solve_tol == NULL_R) then
      vfr_solve_tol = 1.0e-3_r8
    else if (vfr_solve_tol <= 0.0_r8) then
      call TLS_fatal ('VFR_SOLVE_TOL must be > 0')
    end if
    plist => params%sublist('radiation')
    call plist%set('solve-tol', vfr_solve_tol)

    if (vfr_precon_method == NULL_C) then
      vfr_precon_method = 'JACOBI'
    else
      vfr_precon_method = raise_case(vfr_precon_method)
      select case (vfr_precon_method)
      case ('JACOBI')
      case ('CHEBYSHEV')
      case default
        call TLS_fatal('unknown value for VFR_PRECON_METHOD: ' // trim(vfr_precon_method))
      end select
    end if
    plist => params%sublist('radiation')
    call plist%set('precon-method', vfr_precon_method)

    if (vfr_precon_iter == NULL_I) then
      vfr_precon_iter = 1
    else if (vfr_precon_iter < 1) then
      call TLS_fatal ('VFR_PRECON_ITER must be > 0')
    end if
    call plist%set('precon-iter', vfr_precon_iter)

    plist => params%sublist('precon')

    if (vfr_precon_coupling_method /= NULL_C) then
      select case (raise_case(adjustl(vfr_precon_coupling_method)))
      case ('JACOBI')
        vfr_precon_coupling_method = 'jacobi'
      case ('FORWARD GS')
        vfr_precon_coupling_method = 'forward-gs'
      case ('FORWARD-GS')
        vfr_precon_coupling_method = 'forward-gs'
      case ('BACKWARD GS')
        vfr_precon_coupling_method = 'backward-gs'
      case ('BACKWARD-GS')
        vfr_precon_coupling_method = 'backward-gs'
      case ('FACTORIZATION')
        vfr_precon_coupling_method = 'factorization'
      case default
        call TLS_fatal('unknown value for VFR_PRECON_COUPLING_METHOD: ' &
            // trim(vfr_precon_coupling_method))
      end select
      call plist%set('vfr-precon-coupling', vfr_precon_coupling_method)
    end if
