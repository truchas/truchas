...
!! HT_VECTOR_TYPE
!!
!! This module defines the concrete vector type used by the heat transport
...
!! provide persistent workspace for model assembly routines. When view-factor
!! radiation is active, it also stores the owned radiative flux unknowns.
!!
...
contains
  ...
  subroutine init_mold(this, mold)
    ...
    if (allocated(mold%qrad)) then
      call init_mesh(this, mold%mesh, size(mold%qrad))
    else
  ...
  real(r8) function norm1_(this) result(norm)
    use parallel_communication, only: global_sum
    class(ht_vector), intent(in) :: this
    norm = sum(abs(this%hc(:this%mesh%ncell_onP))) &
         + sum(abs(this%tc(:this%mesh%ncell_onP))) &
  ...
  real(r8) function norm2_(this) result(norm)
    use parallel_communication, only: global_sum
    class(ht_vector), intent(in) :: this
    norm = norm2(this%hc(:this%mesh%ncell_onP))**2 &
  ...
  real(r8) function norm_max_(this) result(norm)
    ...
    class(ht_vector), intent(in) :: this
    norm = max(0.0_r8, &
               maxval(abs(this%hc(:this%mesh%ncell_onP))), &
               maxval(abs(this%tc(:this%mesh%ncell_onP))), &
               maxval(abs(this%tf(:this%mesh%nface_onP))))
