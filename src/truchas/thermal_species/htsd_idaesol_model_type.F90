...
!!
!! This module defines the IDAESOL adapter for the coupled thermal/species
!! transport model. It bundles the coupled model, preconditioner, and norm
!! objects behind the abstract interface used by the implicit DAE integrator.
!!
...
!! to be valid at callback boundaries. Coupled thermal/species model and
!! preconditioner routines that need off-process input entries, including
!! temperature, enthalpy, and concentration state data, are responsible for
!! gathering them, and only on-process output entries are significant on return
!! to IDAESOL.
!!
...
module htsd_idaesol_model_type
  ...
  type, extends(idaesol_model), public :: htsd_idaesol_model
    ...
    type(htsd_precon), pointer :: precon => null() ! unowned reference
    type(htsd_norm),   pointer :: norm   => null() ! unowned reference
  contains
    procedure :: init
...
contains
  ...
  subroutine alloc_vector(this, vec)
    ...
    type(htsd_vector), allocatable :: tmp
    allocate(tmp)
