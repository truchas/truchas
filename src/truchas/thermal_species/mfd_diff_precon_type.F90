...
!!   part. Only the on-process entries of V1 and V2 are required to be valid
!!   on entry and only the on-process entries are significant on return.
!!
...
module mfd_diff_precon_type
  ...
  type, public :: mfd_diff_precon
    ...
    type(pcsr_matrix), pointer :: Sff => null() ! owned object
...
contains
  ...
  subroutine apply(this, f1x, f2x)
    ...
    !! workspace and are synchronized here before matrix operations that read
    !! neighboring cell or face values.
    call this%dm%mesh%cell_imap%gather_offp(f1x)
    ...
    !! Back substitution reads face values adjacent to on-process cells.
    call this%dm%mesh%face_imap%gather_offp(f2x)
