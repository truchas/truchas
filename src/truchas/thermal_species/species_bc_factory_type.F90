!!
!! SPECIES_BC_FACTORY_TYPE
!!
!! A concrete factory class that implements methods for creating the abstract
!! boundary condition objects used by the species transport model.
...
!! transport model. Each subroutine has output arguments STAT and
...
module species_bc_factory_type
  ...
  type, public :: species_bc_factory
...
contains
  ...
  subroutine init(this, mesh, params, use_temperature)

    class(species_bc_factory), intent(out) :: this
    ...
    this%mesh => mesh
    this%params => params
    if (present(use_temperature)) this%use_temperature = use_temperature
  ...
  subroutine iterate_list(this, type, comp, proc, stat, errmsg)
    ...
    class(species_bc_factory), intent(in) :: this
    ...
    do while (.not.piter%at_end())
      ...
      if (stat /= 0) exit
      call plist%get('comp', this_comp, stat, errmsg, default=1)
      if (stat /= 0) exit
...
end module species_bc_factory_type
