...
-----------------------------------------------------------------------------
   Purpose:
      ...
      TestCase to run a basic Truchas simulation that is known to fail
  
   Public Interface:
  
      T = TruchasBasicRunFailTest(unittest.TestCase,TruchasBaseTest)
      T.setUp()
      T.shortDescription()
      T.tearDown()
      T.testBasicRunFail()
...
class TruchasBasicRunFailTest(unittest.TestCase,TruchasBaseTest):
	...
	unittest.TestCase.__init__(self,methodName='testBasicRunFail')
	...
	self.outfile     = self.basename+'.brfout'
	...
	self.watcher   = open(logfl,'w')
	self.watcher.write('In BasicRunFail TestCase')
 	self.watcher.write('\n')
 	self.watcher.write('\n')
	...
	self.watcher.write(str)
        ...
        """
	tmp     = 'Copying %s to %s/ ' %(self.outdir, self.logdir)
	...
	self.watcher.write(str)
        ...
        if self.outdir in os.listdir(self.dir):
            try:
                shutil.copytree(self.outdir,self.logdir)
            except:
                cmd = 'cp -r %s %s/. > %s' %(self.outdir, self.logdir, 'stderr_brf')
                os.system(cmd)
                
        shutil.move(self.outfile,self.logdir)
        """
        
    def testBasicRunFail(self):
	"runs a basic Truchas simulation that is expected to fail" 
	"(i.e runs truchas -o:output_dir input_file)"

	tmp     = '*testMethod = testBasicRunFail'
        str     = self.rjustln(self.column1, tmp)
	self.watcher.write(str)
        self.watcher.write('\n')
	...
	self.watcher.write(T.str())
        ...
        #cater for ungraceful truchas exits (mpi process exit) when running with np>1 or on clusters...
        validcodes = [0,1,256,31488]
        
        if retcode not in validcodes:
        ...
        if (self.success in output) :
            outfl = os.path.join(os.getcwd(),self.outfile)
	    tmp   = 'ERROR!! Program %s terminated NORMALLY. This is a failure test!\nSee %s for details!' \
	           % (self.truchas_exe,outfl)
            str   = self.rjustln(self.column2, tmp)
            self.watcher.write('\n')
	    self.watcher.write(str)
            self.watcher.write('\n')
            ...
            for exefile in os.listdir(truchas_bin):
         	if os.path.isfile(os.path.join(truchas_bin,exefile)) and exefile != 'truchas':
        		...
        		version      = vals[4][len(vals[4])-1] + '.' + vals[5]
                        ...
                        for problem in ['box_fill_ortho_fail','ht_bc_fail','shear_constr_heat_fail','static_drop']:
        
                            infile = '%s.inp' %(problem)
                            
                            T = TruchasBasicRunFailTest(currdir,
        				                infile,
        						parallel_env,
        						1,
        						compiler,
