...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "diverging-duct-old-1b.inp")
    xc = output.centroids()
    time = output.time(2)
    ...
    # excessive tolerance but needed for passing
    nfail += truchas.compare_max(verr, 0, 0.6, "y-velocity", time)
    nfail += truchas.compare_max(werr, 0, 0.6, "z-velocity", time)
