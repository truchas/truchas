...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "free-surf-flow-old-2.inp")
    ...
    nfail += velocity_test(output.field(1, "Z_VC"), vof[:,0], 1e-11, output.time(1))
    ...
    nfail += velocity_test(output.field(3, "Z_VC"), vof[:,0], 1e-11, output.time(3))
