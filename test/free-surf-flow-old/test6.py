...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "free-surf-flow-old-6.inp")
    ...
    nfail += truchas.compare_max(output.field(1, "Z_P"), 0, 1e-15, "pressure", output.time(1))
    nfail += velocity_test(output.field(1, "Z_VC"), vof[:,0], 0, output.time(1))

    # intermediate time
    vof = output.field(2, "VOF")
    nfail += vof_test(vof, xc, 5e-2, output.time(2))
    ...
    nfail += vof_test(vof, xc, 7e-2, output.time(3))
    nfail += truchas.compare_max(output.field(3, "Z_P"), 0, 1e-13, "pressure", output.time(3))
