...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "free-surf-flow-old-7.inp")
    golden = tenv.output("free-surf-flow-old-7_golden/free-surf-flow-old-7.h5")
    ...
    uerror = max(abs(u - 2/3*time) if vf > 0.0 else abs(u) for u,vf in zip(test[:,0],vof))
