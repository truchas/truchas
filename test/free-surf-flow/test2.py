...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "free-surf-flow-2.inp")
    ...
    nfail += truchas.compare_max(output.field(1, "Z_P"), 0, 2e-10, "pressure", output.time(1))
    ...
    nfail += truchas.compare_max(output.field(2, "Z_P"), 0, 1e-13, "pressure", output.time(2))
