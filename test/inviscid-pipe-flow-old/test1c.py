...
class mytest(TruchasTest.GoldenTestCase):
  ...
  test_name = 'inviscid-pipe-flow-old-1c'
  ...
  def test_early_velocity(self):
    '''PIPE-FLOW-INVISCID-3: early velocity'''
    ...
    if (error > tol):
      fail += 1
      print 'z-velocity: max rel error = %8.2e: FAIL (tol=%8.2e)'%(error,tol)
    else:
      print 'z-velocity: max rel error = %8.2e: PASS (tol=%8.2e)'%(error,tol)

    self.assertTrue(fail == 0)

  def test_final_velocity(self):
    '''PIPE-FLOW-INVISCID-3: final velocity'''
