...
class mytest(TruchasTest.GoldenTestCase):
  ...
  test_name = 'inviscid-pipe-flow-old-1d'
  ...
  def test_early_velocity(self):
    '''PIPE-FLOW-INVISCID-4: early velocity'''
    ...
    error = max(abs((data[:,1] - vel)/vel))
    if (error > tol):
      fail += 1
      print 'y-velocity: max rel error = %8.2e: FAIL (tol=%8.2e)'%(error,tol)
    else:
      print 'y-velocity: max rel error = %8.2e: PASS (tol=%8.2e)'%(error,tol)
  ...
  def test_final_velocity(self):
    '''PIPE-FLOW-INVISCID-4: final velocity'''
