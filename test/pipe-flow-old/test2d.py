...
class mytest(TruchasTest.GoldenTestCase):
  ...
  def velocity_test(self, id, tol1, tol2):
    ...
    u = (1 - 0.5*(centroids[:,0]-centroids[:,1])**2)/2/math.sqrt(2)
    v = u
    ...
    if (error > tol1):
      fail += 1
      print 'y-velocity at t=%8.2e: max error = %8.2e: FAIL (tol=%8.2e)'%(time,error,tol1)
    else:
      print 'y-velocity at t=%8.2e: max error = %8.2e: PASS (tol=%8.2e)'%(time,error,tol1)
