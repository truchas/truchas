...
def run_test(tenv):
    nfail = 0
    stdout, output = tenv.truchas(4, "steady-flow-old-6b.inp")

    # From a special run to compute vof for final exact configuration
    gold = tenv.output("steady-flow-old-6b_golden/steady-flow-old-6b.h5")

    # Test final VOFs against golden output
    vof_output = output.field(2, "VOF")
    vof_gold = gold.field(1, "VOF")
    nfail += truchas.compare_max(vof_output, vof_gold, 0.3, "VOF", output.time(2))

    # max <= l2 <= sqrt(ncell)*max = 31*max
    nfail += truchas.compare_l2(vof_output, vof_gold, 4*0.3, "VOF", output.time(2))
